<?php 
/* Template Name: Instructor Students Certificate*/
get_header('sleek'); ?>
		...
		<div class="container">
			<div class="row">
				...
				if ( is_user_logged_in() ):
					...
					<?php $id = get_the_ID(); 
					$results = $wpdb->get_results( "SELECT user_id FROM esalSKkOusermeta WHERE meta_key = 'certificates' AND meta_value LIKE '%$id%'");	
					?>
					...
					<?php endwhile; else : ?>
						<p><?php esc_html_e( 'You dont have any student right now so you are not allowed to see the certificates.' ); ?></p>
					<?php endif; ?>
					
					
					<?php if($results == ''){ ?>
					<p><?php echo 'You dont have any student right now so you are not allowed to see the certificates.'; ?></p>
					<?php } 
					/*
					if(is_array($id)){
						foreach($id as $idd){	
