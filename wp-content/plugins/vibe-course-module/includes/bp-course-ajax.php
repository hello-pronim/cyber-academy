...
class BP_Course_Ajax{
    ...
    function get_results_page(){
      ...
      if($the_quiz->have_posts()){
        ...
        if($the_quiz->max_num_pages>1){?>
            ...
            <div class="pag-count">
              <?php echo sprintf(__('Viewing %d out of %d','wplms'),$paged,$the_quiz->max_num_pages) ?>
            </div>
    ...
    function instructor_uncomplete_unit(){
        if (!is_numeric($_POST['course_id']) || !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'security'.$_POST['course_id'])  || !is_numeric($_POST['user_id']) || !is_numeric($_POST['id']) || !current_user_can('edit_posts')){
            echo '<div id="message" class="info notice"><p>'.__('Security check failed !','wplms').'</p></div>';
            die();
        }
      ...
      if(bp_course_is_member($_POST['course_id'],$_POST['user_id'])){
        ...
        $time = apply_filters('wplms_force_complete_unit',time(),$_POST['id'],$_POST['course_id'],$_POST['user_id']);
          ...
          if($post_type == 'unit'){
            ...
            bp_course_update_user_quiz_status($_POST['user_id'],$_POST['id'],3);
            do_action('wplms_quiz_course_retake_reset',$_POST['id'],$_POST['user_id']);
    ...
    function reset_question_answer(){
      ...
      if(isset($ques_id) && $_POST['security'] && wp_verify_nonce($_POST['security'],'security'.$ques_id)){
        ...
        echo '<p>'.__('Answer Reset','wplms').'</p>';
      }else
        echo '<p>'.__('Unable to Reset','wplms').'</p>';
    ...
    function calculate_stats_course(){
        ...
        if ( !isset($course_id) || !$course_id){
        	echo '<p>'.__('Incorrect Course selected.','wplms').'</p>';
            die();
        }
    ...
    if(function_exists('assignment_comment_handle')){ // Assignment is active
      ...
      if(count($assignments)){ // If any connected assignments
    	...
    	if(!$flag){
    		echo '<p>'.__('Statistics successfully calculated. Reloading...','wplms').'</p>';
    	}else{
    		echo '<p>'.__('Unable to calculate Average.','wplms').'</p>';
    	}
    ...
    function course_stats_user(){
      ...
      echo '<a class="show_side link right" data-side=".course_stats_user">'.__('SHOW STATS','wplms').'</a><div class="course_stats_user"><a class="hide_parent link right">'.__('HIDE','wplms').'</a>';


      if ( !isset($user_id) || !$user_id){
      	echo '<div id="message" class="info notice"><p>'.__('Incorrect User selected.','wplms').'</p></div>';
          die();
      }
      ...
      $start = $wpdb->get_var($wpdb->prepare("SELECT date_recorded FROM {$bp->activity->table_name} WHERE type ='start_course' AND item_id=%d AND component='course' AND user_id=%d ORDER BY id DESC LIMIT 1", $course_id,$user_id));
    	...
    	if(empty($being)){
    			echo '<p>'.__('This User has not started the course.','wplms').'</p>';
  		}else{

        if($being > 3){
          $marks = get_post_meta($course_id,$user_id,true);
    			echo '<p>'.__('This User has completed the course.','wplms').'</p>';
    			echo '<h4>'.__('Student Score for Course ','wplms').' : <strong>'.$marks.__(' out of 100','wplms').'</strong></h4>';
        }  
          ...
          echo '</h6>';
    			...
    			$curriculum = '<div class="curriculum_check"><h6>'.__('Curriculum :','wplms').'</h6><ul>';
    			$quiz ='<h5>'.__('Quizes','wplms').'</h5>';
              ...
              if($type == 'unit'){
                ...
                if(is_numeric($marks)){
                    $complete++;
  							    $curriculum .= '<li class="check_user_quiz_results" data-quiz="'.$c.'" data-user="'.$user_id.'"><span class="done" data-id="'.$c.'"></span> '.get_the_title($c).' <strong>'.(($marks)?_x('Marks Obtained : ',' marks obtained in quiz result','wplms').$marks:__('Under Evaluation','wplms')).'</strong></li>';
                    }else{
                        $curriculum .= '<li><span data-id="'.$c.'"></span> '.get_the_title($c).'</li>';    
                    }
                }
      ...
      if(empty($complete)){$complete = 0;}
    	echo '<strong>'.__('Units Completed ','wplms').$complete.__(' out of ','wplms').$total.'</strong>';
  ...
  function wplms_check_user_quiz_results($quiz_id = NULL,$user_id = NULL){
    ...
    do_action('wplms_quiz_results_extras',$quiz_id,$user_id);
    ...
    function remove_user_course(){
        ...
        if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'security'.$course_id) ){
            echo '<div class="message">'.__('Security check failed !','wplms').'</div>';
            die();
        }

        if ( !isset($user_id) || !$user_id){
            echo '<p>'.__(' Incorrect User selected.','wplms').'</p>';
            die();
        }
        ...
        if($students >= 1){
          ...
          update_post_meta($course_id,'vibe_students',$students);
  			...
  			echo '<p>'.__('User removed from the Course','wplms').'</p>';
      ...
      if(isset($status) && is_numeric($status)){  // Necessary for continue course
        ...
        if(isset($user_certifications) && is_Array($user_certifications) && in_array($course_id,$user_certifications)){
            ...
            update_user_meta($user_id,'certificates',$user_certifications);
  			...
  			echo '<p>'.__('Course Reset for User','wplms').'</p>';
          
        do_action('wplms_course_reset',$course_id,$user_id);

    	}else{
    		echo '<p>'.__('There was issue in resetting this course for the user. Please contact admin.','wplms').'</p>';
    	}
    	die();
    }
    ...
    function reset_quiz(){
      ...
      $quiz_id = $_POST['id'];
      $user_id = $_POST['user'];
      do_action('before_wplms_quiz_reset',$quiz_id,$user_id);
      ...
      echo '<p>'.__('Quiz Reset for Selected User','wplms').'</p>';
    ...
    function give_marks(){
      ...
      $from_activity = $_POST['from_activity'];
      if(!empty($from_activity)){
        if(!empty($quiz_id) && !empty($user_id)){
          $results = bp_course_get_quiz_results_meta($quiz_id,$user_id,$from_activity);
          if(!empty($results)){
            if(!is_array($results)){
              $results = unserialize($results);
            }
            if(!empty($results[$answer_id])){
              $results[$answer_id]['marks'] = $value;
              if(isset($results[$answer_id]['raw']) && !empty($results[$answer_id]['raw'])){
                if(is_object($results[$answer_id]['raw'])){
                  $results[$answer_id]['raw']->user_marks =  $value;
                  $results[$answer_id]['raw']->status =  1;
                }else{
                  $results[$answer_id]['raw']['user_marks'] =  $value;
                  $results[$answer_id]['raw']['status'] =  1;
                }
              ...
              bp_course_generate_user_result($quiz_id,$user_id,$results,$from_activity);
            }
          }
        }
    ...
    function complete_course_marks(){
        ...
        echo __('COURSE MARKED COMPLETE','wplms');
    ...
    function wplms_evaluate_course(){
        ...
        echo '<h4>'.sprintf(_x('%s submission by %s','Course submission by User in course - admin - submissions','wplms'),get_the_title($course_id),bp_core_get_user_displayname($user_id)).'</h4>';
        $sum=$max_sum=0;
        $curriculum= bp_course_get_curriculum($course_id);
        ...
        foreach($curriculum as $c){
          if(is_numeric($c)){
            if(bp_course_get_post_type($c) == 'quiz'){
                ...
                $marks=apply_filters('vibe_eval_quiz_marks',get_post_meta($c,$user_id,true),$c,$user_id);
                $sum += intval($marks);
                ...
                $max_sum +=$max;
                echo '<li>
                      <strong>'.get_the_title($c).' <span>'.((isset($status) && $status !='')?__('MARKS: ','wplms').$marks.__(' out of ','wplms').$max:__(' PENDING','wplms')).'</span></strong>
                      </li>';
            }else{
                $status = bp_course_get_user_unit_completion_time($user_id,$c,$course_id);
                echo '<li>
                      <strong>'.get_the_title($c).' <span>'.((isset($status) && $status !='')?'<i class="icon-check"></i> '.__('DONE','wplms'):'<i class="icon-alarm-1"></i>'.__(' PENDING','wplms')).'</span></strong>
                      </li>';
            } 
          }else{
        ...
        echo '</ul>';
        echo '<div id="total_marks">'.__('Total','wplms').' <strong><span>'.apply_filters('wplms_course_student_marks',$sum,$course_id,$user_id).'</span> / '.apply_filters('wplms_course_maximum_marks',$max_sum,$course_id,$user_id).'</strong> </div>';
        echo '<div id="course_marks">'.__('Course Percentage (Out of 100)','wplms').' <strong><span><input type="number" name="course_marks" id="course_marks_field" class="form_field" value="'.(!empty($existing_marks)?$existing_marks:'0').'" placegolder="'.__('Course Percentage out of 100','wplms').'" /></span></div>';
        echo '<a href="#" id="course_complete" class="button full" data-course="'.$course_id.'" data-user="'.$user_id.'">'.__('Mark Course Complete','wplms').'</a>';
    ...
    function evaluate_quiz(){
      ...
      if(bp_course_get_post_type($quiz_id) != 'quiz'){
        echo '<p>'.__(' Incorrect Quiz Id.','wplms').'</p>';
          die();
      }
      ...
      $yesfromactivity = apply_filters('wplms_quiz_evaluate_from_activity',true,$quiz_id,$user_id);

      if(function_exists('bp_is_active') && bp_is_active('activity') && $yesfromactivity){
      ...
      if(!empty($results)){
        //from activity meta
        echo '<h3 class="evaluate_heading">'.get_the_title($quiz_id).' '._x('submission by','Quiz submission "by" user','wplms').' '.bp_core_get_user_displayname($user_id).'</h3>';
        if(count($results)):
          ...
          foreach($results as $question_key=>$question){
            ...
            echo '<li>
              ...
              $marked_answer = $question['marked_answer'];
              $marked_answer_id = $question_key;
              $marks = $question['marks'];
              echo '<strong>';_e('Marked Answer :','wplms');echo '</strong>';
              echo $marked_answer;
              ...
              if(isset($correct_answer) && $correct_answer !='' && isset($marked_answer) && $marked_answer != ''){
                  echo '<strong>';_e('Correct Answer :','wplms');echo '<span>'.$correct_answer.'</span></strong>';
              }

              if(!empty($marks)){
                if(!isset($results[$marked_answer_id]))
                    echo '<strong>'._x('Unattempted question ! Marks can not be assigned for this question.','Unattempted question warning in Course - Admin - Submissions area','wplms').'</strong>';
                else
                    echo '<span class="marking">'.__('Marks Obtained','wplms').' <input type="text" id="'.$marked_answer_id.'" class="form_field question_marks" value="'.$marks.'" placeholder="'.__('Give marks','wplms').'" />
                    <a href="#" class="give_marks button" data-from-activity="'.$activity_id.'" data-ans-id="'.$marked_answer_id.'" data-quiz="'.$quiz_id.'" data-user="'.$user_id.'">'.__('Update Marks','wplms').'</a>';
                ...
                if(!isset($results[$marked_answer_id])){
                    ...
                    echo '<span class="marking">'.__('Marks Obtained','wplms').' <input type="text" id="'.$marked_answer_id.'" class="form_field question_marks" value="" placeholder="'.__('Give marks','wplms').'" />
                    <a href="#" class="give_marks button" data-from-activity="'.$activity_id.'" data-ans-id="'.$marked_answer_id.'" data-quiz="'.$quiz_id.'" data-user="'.$user_id.'">'.__('Give Marks','wplms').'</a>';
                }
              }
            $max_sum=$max_sum+intval($question['max_marks']);
            echo '<span> '.__('Total Marks','wplms').' : '.$question['max_marks'].'</span>';
            echo '</li>';
...
// return;
              ...
              if(isset($correct_answer) && $correct_answer !='' && isset($ans) && $ans != ''){
                  echo '<strong>';_e('Correct Answer :','wplms');echo '<span>'.$ans.'</span></strong>';
              }
              ...
              if(!empty($marks)){
                  ...
                  if(empty($marked_answer_id))
                      ...
                      echo '<span class="marking">'.__('Marks Obtained','wplms').' <input type="text" id="'.$marked_answer_id.'" class="form_field question_marks" value="'.$marks.'" placeholder="'.__('Give marks','wplms').'" />
                      <a href="#" class="give_marks button" data-ans-id="'.$marked_answer_id.'">'.__('Update Marks','wplms').'</a>';
                  ...
                  echo '<span class="marking">'.__('Marks Obtained','wplms').' <input type="text" id="'.$marked_answer_id.'" class="form_field question_marks" value="" placeholder="'.__('Give marks','wplms').'" >
                  <a href="#" class="give_marks button" data-ans-id="'.$marked_answer_id.'">'.__('Give Marks','wplms').'</a>';
              }
            }
            $max_sum=$max_sum+intval($questions['marks'][$key]);
            echo '<span> '.__('Total Marks','wplms').' : '.$questions['marks'][$key].'</span></span>';
            do_action('wplms_quiz_evaluate_per_question_html',$question,$quiz_id,$user_id,$marked_answer_id);
            echo '</li>';
      ...
      if(!empty($questions) || !empty($results)):
        echo '<div id="total_marks">'.__('Total','wplms').' <strong><span>'.$sum.'</span> / '.$max_sum.'</strong> </div>';
        echo '<div id="quiz_remarks_block">';
        echo '<h3 class="heading"><span>'.__('Instructor remarks','wplms').'</span></h3>';
        ...
        echo '<a href="#" id="mark_complete" class="button full" data-from-activity="'.$activity_id.'" data-quiz="'.$quiz_id.'" data-user="'.$user_id.'">'.__('Mark Quiz as Checked','wplms').'</a>';

        do_action('wplms_quiz_evaluate_question_html',$quiz_id,$user_id);
    ...
    */
    ...
    function search_course_members(){
        ...
        if(empty($flag)){
            ...
            $user_ids = $wpdb->get_results(apply_filters('wplms_usermeta_direct_query',$query));
            ...
            if(empty($user_ids)){
                ...
                echo '<li><div class="pagination"><div><div class="pag-count" id="course-member-count">'.sprintf(__('Viewing page %d of %d ','wplms'),(empty($page)?1:$page ),$max_page).'</div>';
        ...
        if($flag){
            echo '<li><div class="message">'._x('No user found !','no user found in live search in course admin','wplms').'</div></li>';
        }
    ...
    function send_bulk_message(){
        ...
        if(!isset($subject)){
          _e('Set a Subject for the message','wplms');
          die();  
        }
        ...
        if(count($members) > 0){
          ...
          echo __('Messages Sent to ','wplms').$sent.__(' members','wplms');
        }else{
          echo __('Please select members','wplms');
        }
        ...
        foreach($members as $member){
          ...
          if(!empty($user_id)){
            ...
            if($check){
                ...
                <li><a class="tip reset_course_user" data-course="'.$course_id.'" data-user="'.$user_id.'" title="" data-original-title="'.__('Reset Course for User','wplms').'"><i class="icon-reload"></i></a></li>
                <li><a class="tip course_stats_user" data-course="'.$course_id.'" data-user="'.$user_id.'" title="" data-original-title="'.__('See Course stats for User','wplms').'"><i class="icon-bars"></i></a></li>
                <li><a class="tip remove_user_course" data-course="'.$course_id.'" data-user="'.$user_id.'" title="" data-original-title="'.__('Remove User from this Course','wplms').'"><i class="icon-x"></i></a></li>
                </ul></li>'; 
                $member_ids[]=$user_id;
            }
          }
    ...
    function assign_badge_certificates(){
        ...
        if(count($members) > 0){
          ...
          if($flag){
            echo __('Action assigned to ','wplms').$assigned.__(' members','wplms');
            
            do_action('wplms_bulk_action',$assign_action,$course_id,$members);

          }else
            echo __('Could not assign action to members','wplms');
    ...
    function wplms_extend_course_subscription(){
        ...
        if(!isset($extend_amount) || !$extend_amount){
          echo __('Please enter extension amount','wplms');
          die();
        }
        ...
        if($neg){
          echo sprintf(__('Subscription extended for %d students, unable to extend for %d students','wplms'),$count,$neg);
        }else{

          echo sprintf(__('Subscription extended for %d students','wplms'),$count);
...
/*=== Manage Course Status v 1.9.5 =====*/
    ...
    function wplms_change_course_status(){
        ...
        if(count($members) > 0){
          ...
          if($flag){
            echo __('Course status changed for ','wplms').$assigned.__(' members','wplms');
...
/*=== DOWNLOAD STATS =====*/
    ...
    function wplms_course_download_stats(){
      ...
      if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'security') ){
          echo __('Security check failed !','wplms');
          die();
      } 
      if (!current_user_can('edit_posts') || !is_numeric($course_id)){
          echo __('User does not have capability to download stats !','wplms');
          die();
      }
            ...
            switch($field){
              ...
              case 'stats_student_name':
                ...
                if(!in_array($title,$csv_title))
				  ...
				  //var_dump(bp_core_get_user_displayname($user_id));
              ...
              case 'stats_student_certificate':
				...
				//echo "SELECT COUNT(meta_key) as count FROM {$wpdb->usermeta} WHERE meta_key = %s AND user_id = %d AND meta_value LIKE %s",'certificates',$user_id,"%$course_id%";
    ...
    function wplms_download_mod_stats(){
      ...
      if(in_array($post_type,array('quiz','wplms-assignment'))){
        switch($type){
          ...
          break;
          case 'finished_students':
            $users = $wpdb->get_results(apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT meta_key as user_id FROM {$wpdb->postmeta} WHERE post_id = %d AND meta_value REGEXP '^[0-9]+$'  AND meta_key IN (SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key = %d AND meta_value < %d)",$id,0,$id,time())),ARRAY_A);
          break;
        }
      }
                ...
                if(!vibe_validate($quiz_dynamic)){
                    ...
                    if(is_array($questions) && is_array($questions['ques'])){
                      ...
                      foreach($questions['ques'] as $m=>$question){
                        ...
                        $user_quiz_result = unserialize($user_quiz_result);
                        if(!empty($user_quiz_result) && !empty($user_quiz_result[$question])){
                          $content = strip_tags($user_quiz_result[$question]['marked_answer']);
                        }else{
...
/* == Display and download stats for Quiz/Question/Assignment == */
    ...
    function wplms_load_cpt_stats(){
      ...
      if($flag){
        echo '<div class="message stats_content"><p>'.__('User not allowed to access stats','wplms').'</p></div>';
        die();
      }
      ...
      if(!is_array($results)){
        echo '<div class="message stats_content"><p>'.__('No data available','wplms').'</p></div>';
        die();
      }
      ...
      if(is_array($user_marks)){
        ...
        echo '
            ...
            <h4>'.__('Average','wplms').'<span class="right">'.$average.'</span></h4>
            <h4>'.__('Max','wplms').'<span class="right">'.$max.'</span></h4>
            <h4>'.__('Min','wplms').'<span class="right">'.$min.'</span></h4>';
      }else{
        echo '<div class="message">'.__('N.A','wplms').'</div>';
      }

      echo '<h3 class="heading"><span>'.__('Leaderboard','wplms').'</span></h3>';
      ...
      if(is_array($user_marks)){
        ...
        echo '</ol>';
        echo '<a id="load_more_cpt_user_results" data-starting_point="'.($starting_point+$count).'" class="link" style="text-align: center;width: 100%;display: inline-block;">'.__('Load more','wplms').'</a>';
      }
      if(is_user_logged_in()){
        $user_id = get_current_user_id();
        $instructors = apply_filters('wplms_course_instructors',get_post_field('post_author',$id,'raw'),$id);
        if((is_array($instructors) && in_array($user_id,$instructors)) || current_user_can( 'manage_options' )){
          echo '<h3 class="heading" id="download_stats_options">'.__('Download Stats','wplms').'<i class="icon-download-3 right"></i></h3>';
          ...
          <option value="all_students">'.__('All students','wplms').'</option>
          <option value="finished_students">'.__('Students who finished','wplms').'</option>
          </select>';
          wp_nonce_field('security','stats_security');
          echo '<a class="button full" id="download_mod_stats" data-type="'.$type.'" data-id="'.$id.'"><i class="icon-download-3"></i> '.__('Process Stats','wplms').'</a>
          </div>';
        } 
      }
      echo '</div>';
      ...
      switch($type){
        ...
        case 'wplms-assignment':
          ...
          echo json_encode(array(
            'labels'=>array(__('Less than 25','wplms'),
              __('More than 25 Less than 50','wplms'),__('More than 50 Less than 75','wplms'),
              __('More than 75 Less than 100','wplms')
            ),
    ...
    function unit_traverse(){
      ...
      if(!isset($_COOKIE['course'])) {
        if($coursetaken>time()){
          ...
          echo '<div class="message"><p>'.__('Course Expired.','wplms').'<a href="'.$pid.'" class="link alignright">'.__('Click to renew','wplms').'</a></p></div>';
          die();
        }
      }
      ...
      if(isset($coursetaken) && $coursetaken){
        ...
        if(!bp_course_check_unit_complete($unit_id,$user_id,$course_id)){ // IF unit not completed by user
                ...
                if($unitkey == 0){ // Start of Course
                    ...
                    if(!isset($pre_unit_time) || $pre_unit_time ==''){
                        ...
                        if(!empty($pre_unit_time)){
                             ...
                             if($value > time()){
                                  $element = apply_filters('wplms_drip_feed_element_in_message',__('Unit','wplms'),$course_id);
                                  echo '<div class="message"><p>'.sprintf(__('%s will be available in %s','wplms'),$element,tofriendlytime($value-time())).'</p></div>';
                                  echo do_shortcode('[countdown_timer event="drip_feed_end" event_detail="'.$units[$unitkey].'" seconds="'.($value-time()).'" size="3"]');
                                  die();
                                }else{
                                ...
                                if(isset($pre_unit_key)){
                                  ...
                                  if(!empty($completed)){
                                      ...
                                      $element = apply_filters('wplms_drip_feed_element_in_message',__('Unit','wplms'),$course_id);
                                      echo '<div class="message"><p>'.sprintf(__('%s can not be accessed.','wplms'),$element).'</p></div>';
                                    }
                              }else{
                                $element = apply_filters('wplms_drip_feed_element_in_message',__('Unit','wplms'),$course_id);
                                echo '<div class="message"><p>'.sprintf(__('%s can not be accessed.','wplms'),$element).'</p></div>';
          ...
          $bppost_type = bp_course_get_post_type($unit_id);
          echo '<div id="unit" class="'.$bppost_type.'_title '.($bppost_type=='wplms-assignment'?'unit_title':'').'" data-unit="'.$unit_id.'">';
          ...
          echo '</div>';
                  ...
                  $quiz_passing_flag = apply_filters('wplms_next_unit_access',true,$units[($k)],$user_id);
                  echo  '<div class="unit_prevnext"><div class="col-md-3">';
                  if($prev >=0){

                    if(bp_course_get_post_type($units[$prev]) == 'quiz'){
                      echo '<a href="#" id="prev_quiz" data-unit="'.$units[$prev].'" class="unit '.$unit_class.'"><span>'.__('Previous Quiz','wplms').'</span></a>';
                    }else{
                      if(get_post_type($units[$prev]) == 'unit'){
                        echo '<a href="#" id="prev_unit" data-unit="'.$units[$prev].'" class="unit unit_button"><span>'.__('Previous Unit','wplms').'</span></a>';
                      }
                      if(get_post_type($units[$prev]) == 'wplms-assignment'){
                        echo '<a href="#" id="prev_unit" data-unit="'.$units[$prev].'" class="unit unit_button"><span>'.__('Previous Assignment','wplms').'</span></a>';
                      }
                    }
                    do_action('wplms_after_previous_button_course_status',$units[$prev],$unit_id,$units,$user_id,$course_id);
                  }
                  echo '</div>';

                  echo  '<div class="col-md-6">';
                  if(bp_course_get_post_type($units[($k)]) == 'quiz'){
                   echo '<a href="'.get_permalink($units[($k)]).'" class=" unit_button '.$quiz_class.'"><span>'.__('Start Quiz','wplms').'</span></a>';
				   /*
				   if(empty($react_quizzes)){
                     ...
                     $quiz_status = get_user_meta($user_id,$units[($k)],true);
                      ...
                      if(is_numeric($quiz_status)){
                        ...
                        if($quiz_status < time()){
                          echo '<a href="'.bp_loggedin_user_domain().BP_COURSE_SLUG.'/'.BP_COURSE_RESULTS_SLUG.'/?action='.$units[($k)].'" class="quiz_results_popup"><span>'.__('Check Results','wplms').'</span></a>';
                        }else{
                            $quiz_class = apply_filters('wplms_in_course_quiz','');
                            echo '<a href="'.get_permalink($units[($k)]).'" class=" unit_button '.$quiz_class.' continue">'.__('Continue Quiz','wplms').'</a>';
                        }
                      }else{
                          $quiz_class = apply_filters('wplms_in_course_quiz','');
                          echo apply_filters('wplms_start_quiz_button','<a href="'.get_permalink($units[($k)]).'" class=" unit_button '.$quiz_class.'"><span>'.__('Start Quiz','wplms').'</span></a>',$units[($k)]);
                      }
                    }
                    */
                  }else  
                      echo ((isset($done_flag) && $done_flag)?'': apply_filters('wplms_unit_mark_complete','<a href="#" id="mark-complete" data-unit="'.$units[($k)].'" class="unit_button"><span>'.__('Mark this Unit Complete','wplms').'</span></a>',$unit_id,$course_id));
                  ...
                  if($next <= $max){
                      ...
                      if(isset($done_flag) && $done_flag && $quiz_passing_flag){
                            $unit_class .=' ';
                            $hide_unit=0;
                      }else{
                            $unit_class .=' hide';
                    ...
                    if(bp_course_get_post_type($units[$next]) == 'quiz'){
                            ...
                            echo '<a href="#" id="next_quiz" '.(($hide_unit)?'':'data-unit="'.$units[$next].'"').' class="unit '.$unit_class.'"><span>'.__('Proceed to Quiz','wplms').'</span></a>';
                    }else {
                       
                            if(get_post_type($units[$next]) == 'unit'){
                              echo '<a href="#" id="next_unit" '.(($hide_unit)?'':'data-unit="'.$units[$next].'"').' class="unit '.$unit_class.'"><span>'.__('Next Unit','wplms').'</span></a>';
                            }
                            if(get_post_type($units[$next]) == 'wplms-assignment'){
                              echo '<a href="#" id="next_unit" '.(($hide_unit)?'':'data-unit="'.$units[$next].'"').' class="unit '.$unit_class.'"><span>'.__('Next Assignment','wplms').'</span></a>';
                            }
                        
                    }
                    do_action('wplms_after_next_button_course_status',$units[$prev],$unit_id,$units,$user_id,$course_id); 
...
/* ===== In Course Quiz ===== */
    ...
    function incourse_submit_quiz(){
      ...
      $user_id = get_current_user_id();
      $qstatus = bp_course_get_user_quiz_status($user_id,$quiz_id);
      
      if(!empty($qstatus) && $qstatus > 2){
          echo _x('Quiz Already Submitted.','','wplms');
          die();
      }

      $access = get_user_meta($user_id,$quiz_id,true);
      if(!isset($access) || !is_numeric($access)){
        _e('Invalid submission time.','wplms');
         die();
      }
      ...
      $flag = apply_filters('wplms_next_unit_access',true,$quiz_id,$user_id);
...
*/
    function submit_quiz(){
      ...
      if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'start_quiz') ){
          _e('security check failed.','wplms');
          die();
      }
      $qstatus = bp_course_get_user_quiz_status($user_id,$quiz_id);
      if(!empty($qstatus) && $qstatus > 2){
    ...
    function begin_quiz(){
        ...
        if ( isset($_POST['start_quiz']) && wp_verify_nonce($_POST['start_quiz'],'start_quiz') ){
          ...
          if(!$quiz_access_flag){
            _ex('Quiz already in progress in App, contact site administrator to proceed','quiz lock flag check for App and Site','wplms');
            die();
          }
          ...
          $quiztaken=get_user_meta($user_id,$id,true);
           ...
           if(!isset($quiztaken) || !$quiztaken){
            ...
            if($quiztaken > time()){
              ...
              echo '<div class="message error"><h3>'.__('Quiz Timed Out .','wplms').'</h3>'; 
              echo '<p>'.__('If you want to attempt again, Contact Instructor to reset the quiz.','wplms').'</p></div>
          ...
          echo '<h3>'.__('Quiz Already Attempted.','wplms').'</h3>'; 
          echo '<p>'.__('Security Check Failed. Contact Site Admin.','wplms').'</p>'; 
       }
       die();
    }  
...
// After begin quiz, get unmarked quesitons
      ...
      function quiz_question(){
          ...
          if ( isset($_POST['start_quiz']) && wp_verify_nonce($_POST['start_quiz'],'start_quiz') ){ // Same NONCE just for validation
            ...
            echo '<p>'.__('If you want to attempt again, Contact Instructor to reset the quiz.','wplms').'</p></div>';
                ...
                echo '<div class="message info"><h3>'.__('Start Quiz to begin quiz.','wplms').'</h3>'; 
                echo '<p>'.__('Click "Start Quiz" button to start the Quiz.','wplms').'</p></div>';
             }

         }else{
                    echo '<div class="message error"><h3>'.__('Quiz not active.','wplms').'</h3>'; 
                    echo '<p>'.__('Contact your instructor or site admin.','wplms').'</p></div>';
         }
         die();
      }  
          ...
          if ( isset($_POST['start_quiz']) && wp_verify_nonce($_POST['start_quiz'],'start_quiz') ){ // Same NONCE just for validation
           if(isset($quiztaken) && $quiztaken && $quiztaken > time()){
            ...
            echo '<p>'.__('If you want to attempt again, Contact Instructor to reset the quiz.','wplms').'</p></div>';
              ...
              echo '<div class="message error"><h3>'.__('Quiz Already Attempted .','wplms').'</h3>'; 
              echo '<p>'.__('If you want to attempt again, Contact Instructor to reset the quiz.','wplms').'</p></div>'; 
         }
          die();
      }  
    ...
    function incourse_start_quiz(){
      ...
      if(!$quiz_access_flag){
        _ex('Quiz already in progress, contact site, please retry after sometime.','quiz lock flag check for App and Site','wplms');
        die();
      }
      ...
      if ( $the_query->have_posts() ) {
        ...
        while ( $the_query->have_posts() ) {
          ...
          if(isset($hint) && strlen($hint)>5){
            echo '<a class="show_hint tip" tip="'.__('SHOW HINT','wplms').'"><span></span></a>';
            echo '<div class="hint"><i>'.__('HINT','wplms').' : '.apply_filters('the_content',$hint).'</i></div>';
          }
          echo '</div>';
          ...
          switch($type){
            ...
            break;
            case 'smalltext': 
              echo '<input type="text" name="'.$k.'" class="ques'.$k.' form_field" value="'.($content?$content:'').'" placeholder="'.__('Type Answer','wplms').'" />';
            break;
            case 'largetext': 
              echo '<textarea name="'.$k.'" class="ques'.$k.' form_field" placeholder="'.__('Type Answer','wplms').'">'.($content?$content:'').'</textarea>';
            break;
            default:
         ...
         $count = count($questions);
          if($posts_per_page < $count){
              echo '<div class="pagination"><label>'.__('PAGES','wplms').'</label>
              <ul>';
    ...
    function wplms_add_unit_comment(){
      ...
      if(strlen($content) < 2){
        echo '<div class="message">'.__('Unable to add message','wplms').'</div>';
        die();
      }
                ...
                <div id="div-comment-'.$comment_id.'" class="unit-comment-body '.(empty($approved_status)?'unapproved':'approved').'">
                  ...
                  <div class="unit-comment-text">  
                    ...
                    <div class="comment-meta commentmetadata">
                      <a href="'.get_permalink($unit_id).'">'.__('JUST NOW','wplms').'</a>'.(empty($approved_status)?' &nbsp; <a>( '._x('Sent for Approval.','when unit comment is not approved','wplms').' ) </a>':'').'  
                    </div>
    ...
    function fetch_quiz_submissions(){
      ...
      switch($status){
        case 0:
         //for pending
        $datas = $wpdb->get_results( apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT DISTINCT u.user_id as meta_key FROM {$wpdb->postmeta} as p LEFT JOIN {$wpdb->usermeta} as u ON p.meta_key = u.user_id WHERE p.meta_value LIKE '0' AND u.meta_key = p.post_id AND u.meta_value < %d AND p.post_id = %d",time(),$quiz_id)), ARRAY_A); // Internal Query
        if(!empty($datas)){
          $i = 0;
          foreach($datas as $data){
            $quiz_status = bp_course_get_user_quiz_status($data['meta_key'],$quiz_id);
            if($quiz_status != 4 ) {
              $members[$i]['meta_key'] = $data['meta_key'];
              $i++;
            }
          } 
        }
        ...
        //for evaluation completed
          $members = $wpdb->get_results(apply_filters('wplms_usermeta_direct_query', $wpdb->prepare("SELECT DISTINCT u.user_id as meta_key FROM {$wpdb->postmeta} as p LEFT JOIN {$wpdb->usermeta} as u ON p.meta_key = u.user_id WHERE p.meta_value NOT LIKE '0' AND u.meta_key = p.post_id AND u.meta_value < %d AND p.post_id = %d",time(),$quiz_id)), ARRAY_A); // Internal Query
        break;
      }
      ...
      if(!empty($members)){
        ...
        foreach($members as $member ){
            ...
            if(is_numeric($member['meta_key'])){
              ...
              echo '</span>';
              echo '<ul> 
                  <li><a class="tip reset_quiz_user" data-quiz="'.$quiz_id.'" data-user="'.$member_id.'" title="'.__('Reset Quiz for User','wplms').'"><i class="icon-reload"></i></a></li>
                  <li><a class="tip evaluate_quiz_user" data-quiz="'.$quiz_id.'" data-user="'.$member_id.'" title="'.__('Evaluate Quiz for User','wplms').'"><i class="icon-check-clipboard-1"></i></a></li>
                  </ul>';
              echo '</li>';
            }
          } 
        ...
        echo '<div class="message">'._x('No submissions found','Quiz submission not found','wplms').'</div>';
      }
      echo '</ul>';
    ...
    function fetch_course_submissions(){
        ...
        if($_POST['status'] == 4){
          $members_submit_course = $wpdb->get_results( apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT user_id as meta_key FROM {$wpdb->usermeta} where meta_key = %s AND meta_value = %d",'course_status'.$course_id,4)), ARRAY_A); // Internal Query
        }else{
          $members_submit_course = $wpdb->get_results( apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT user_id as meta_key FROM {$wpdb->usermeta} where meta_key = %s AND meta_value = %d",'course_status'.$course_id,3)), ARRAY_A); // Internal Query
        }
        ...
        if(!empty($members_submit_course)){
          ...
          foreach($members_submit_course as $submit_course ){
            ...
            echo '<li id="s'.$member_id.'">';
              ...
              echo '</span>';
              echo '<ul> 
                  <li><a class="tip evaluate_course_user" data-course="'.$course_id.'" data-user="'.$member_id.'" title="'.__('Evaluate Course for User','wplms').'"><i class="icon-check-clipboard-1"></i></a></li>
                  </ul>';
              echo '</li>';
            }
          }
          ...
          echo '<div class="message">'._x('No course submissions !','No course submissions found in Course - Admin - submissions seciton','wplms').'</div>';
        }
        die();
    }
    ...
    function get_unit_content(){ 
        ...
        $user_id = get_current_user_id();
        $coursetaken=bp_course_get_user_expiry_time($user_id,$course_id);
        if($course_taken < time()){
            _e('Course Expired','wplms');
            die();
        }
    ...
    function apply_for_course(){
      ...
      echo _x('Applied for Course','Apply for Course confirmation message.','wplms');
    ...
    function check_question_answer(){
        ...
        if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],$security)){
            echo json_encode(array('message'=>__('Security check Failed. Contact Administrator.','wplms')));
            die();
        }

        $marks = bp_course_get_user_question_marks($quiz_id,$question_id,$user_id);
        if(is_numeric($marks)){
          echo json_encode(array('message'=>__('Question answer already marked','wplms')));
          die();
        }
        ...
        if(isset($questions) && isset($questions['ques'])){
            if(!in_array($question_id,$questions['ques'])){
                echo json_encode(array('message'=>__('Question not included in quiz.','wplms')));
                die();       
            }
        }else{
            echo json_encode(array('message'=>__('Unable to fetch quiz questions.','wplms')));
            die();       
        }
        ...
        if(empty($correct)){
            ...
            update_question_incorrect_percentage($question_id);
        }else{
          ...
          update_question_correct_percentage($question_id);
        }
        ...
        if(isset($correct_answer)){
          if(in_Array($type,array('single','select'))){
              ...
              if($type == 'select' && strpos($correct_answer,'|') !== false){
                ...
                if(!empty($corr_ans)){
                  ...
                  foreach($corr_ans as $ca){
                    if($return['correct']){$return['correct'] .=' '.__('and','wplms').' ';}
            ...
            if($correct_answer){$return['correct']=__('True','wplms');}else{$return['correct']=__('False','wplms');}
