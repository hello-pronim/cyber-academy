...
class BP_Course_Action{
    ...
    private function __construct(){
        ...
        add_action('bp_course_header_actions',array($this,'wplms_get_course_schema_details'));
        ...
        */
        ...
        add_action('wp_head',array($this,'add_data_for_scorm'));
        add_action('wplms_unit_header',array($this,'add_data_for_scorm_unit_header'),10,2);
    ...
    function add_data_for_scorm_unit_header($unit_id,$course_id){

        if(!is_user_logged_in())
            return;
        $user = wp_get_current_user();
        $type = get_post_type($unit_id);
        $user_id = $user->ID;
        $token = '';
        $tokens  = get_user_meta($user_id,'access_tokens',true);
        ...
        <script>
            ...
            var scorm_wplms_data = <?php echo json_encode($json_data);?>;
            
        </script>
        <?php
    }

    function add_data_for_scorm(){
        global $post;
        if(!is_user_logged_in())
            return;
        $post_types = apply_filters('wplms_add_data_for_scorm',array('unit','quiz'),$post);
    ...
    function show_quiz_results_after_quiz_message($quiz_id,$user_id){
        ...
        if(function_exists('vibe_validate') && vibe_validate($eanbled)){
            ...
            if(!empty($quiz_id) && !empty($user_id)){
                ...
                $return .= '<style>.quiz_result h3.heading span,.quiz_results_popup {display:none !important;}</style><script>jQuery(".quiz_result").find(".live-edit").html("________________");</script>';
            }
        }
        echo $return;
    }
    ...
    function show_retake_below_course_button($user_id,$course_id,$course_user){
        ...
        if(!empty($course_id) && bp_course_is_member($course_id,$user_id) && $course_user >= 4 ){
            ...
            if((intval($retakes) - count($course_retakes)) > 0){
    ...
    function unit_content(){
        ...
        if(class_exists('WPLMS_tips')){
          ...
          if(isset($tips) && isset($tips->settings) && !empty($tips->settings['react_quizzes'])){
                ...
                if(get_post_type($this->status_units[($k)]) == 'quiz'){
                    if(empty($react_quizzes)){
                        ...
                        if(is_numeric($quiz_status)){
                            ...
                            echo apply_filters('wplms_start_quiz_button','<a href="'.get_permalink($this->status_units[($k)]).'" class="mink-hide unit_button '.$quiz_class.'" data-quiz="'.$this->status_units[($k)].'"> <span>'.__('Start Quiz','vibe').'</span></a>',$this->status_units[($k)]);
							echo '<a href="'.get_permalink($this->status_units[($k)]).'" class="unit_button '.$quiz_class.'" data-quiz="'.$this->status_units[($k)].'"> <span>'.__('Start Quiz','vibe').'</span></a>';
                        }
                    }
                }else{
    ...
    */
    ...
    function wplms_get_course_schema_details(){
        global $post;
        ...
        if(empty($meta) || false === $meta){
            $meta = wplms_get_course_schema_details($post->ID);
            $meta = '<script type="application/ld+json">'.json_encode($meta).'</script>';
    ...
    function bp_course_schema(){
        ...
        if(empty($meta) || false === $meta){
            ...
            <div itemscope itemtype="http://schema.org/Product">
                ...
                <meta itemprop="productID" content="<?php echo $post->ID; ?>">
    ...
    function get_quiz_item($post,$user_id){
        $value = apply_filters('get_quiz_item_value',get_post_meta($post->ID,$user_id,true),$post->ID,$user_id);
