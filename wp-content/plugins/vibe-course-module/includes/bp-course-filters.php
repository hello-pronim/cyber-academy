...
class bp_course_filters{
    ...
    private function __construct(){
		...
		add_action('template_redirect',function(){
			if(is_singular('quiz')){
				add_filter('wplms_start_quiz_button',array($this,'quiz_check_unit_locking'),10,2);		
			}
		});
    ...
    function quiz_check_unit_locking($button, $quiz_id){
    	...
    	if(is_user_logged_in()){
	    	...
	    	if($lock === 'S'){
	    		...
	    		$index = array_search(get_the_ID(), $curriculum);
	    		if(!empty($curriculum) && $index > 0){
	    			foreach($curriculum as $key=>$unit_id){
	    				if($key < $index){

	    					$completed = bp_course_get_user_unit_completion_time(get_current_user_id(),$unit_id,$course_id);
	                          if(empty($completed)){
	                          	return '<a class="button create-group-button full" href="'.get_permalink($course_id).'">'._x('Unfinished Previous Units','unifinished units','vibe').'</a>';
	                          }
	    				}
	    			}
	    		}
   ...
   return $content;
	...
	<?php else: ?>
		...
		<div id="message" class="info">
			<p><?php _e( 'You are not instructing any course right now. You will be able to see the course(s) only when course(s) will be instructing by you.', 'vibe' ); ?></p>
		</div>
