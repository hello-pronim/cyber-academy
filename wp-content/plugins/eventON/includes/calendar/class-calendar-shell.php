	 ...
	 */
		public function get_supported_shortcode_atts(){
			$args = array(
				...
				'eventtop_style' => 0, // event type style
			);
	 ...
	 */
		...
		function get_cal_shortcode_args(){
			...
			$output =  apply_filters('eventon_calhead_shortcode_args', array(
				...
				'hide_ft'=>((!empty($arg['hide_ft']))? $arg['hide_ft']:'no'),
				...
				'hide_end_time'	=>(!empty($arg['hide_end_time'])?$arg['hide_end_time']:'no'),
				'eventtop_style'	=>(!empty($arg['eventtop_style'])?$arg['eventtop_style']:0),
			), $arg);
