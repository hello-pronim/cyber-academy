		...
		switch($box_f){
			...
			// TIME and LOCATION
				case 'timelocation':
					...
					if($object->address || $object->location_name){
						...
						$locationLink = (!empty($object->location_link))? '<a target="'. $object->location_link_target .'" href="'. evo_format_link($object->location_link).'">':false;
				...
				case 'locImg':
					...
					if(!empty($img_src)){
						...
						if(!empty($object->locName)){
							...
							if(!empty($object->locAdd))	$inner .= '<span style="padding-bottom:10px">'. stripslashes($object->locAdd) .'</span>';
			...
			// GOOGLE map
				case 'gmap':					
					...
					//$OT.= "<div class='evo_metarow_osmap' style='display:block;height:300px' id='".$object->id."_osmap'></div>";
				break;
				...
				case 'organizer':					
					...
					$OT.= "<div class='evo_metarow_organizer evorow evcal_evdata_row bordb evcal_evrow_sm ".$end_row_class."'>
							...
							<div class='evcal_evdata_cell'>							
								...
								$description = $object->organizer_term->description? stripslashes($object->organizer_term->description): false;
								...
								$org_data = "<p class='evo_data_val evo_card_organizer_name'>
									...
									(!empty($ORGMeta['evcal_org_contact'])? 
