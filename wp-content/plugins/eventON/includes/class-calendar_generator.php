...
class EVO_generator {
	...
	/**	Construction function	 */
		public function __construct(){
			...
			include_once('calendar/class-calendar-shell.php');
			include_once('calendar/class-calendar-body.php');
			include_once('calendar/class-calendar-filtering.php');
			...
			function separate_eventlist_to_months($eventList, $eventCount = false, $args){
				...
				if(sizeof($eventList)){
					...
					foreach($sorted_list as $moyr=>$events){
						...
						// set global calendar language @+ 2.6.11
						evo_set_global_lang( $args['lang']); 
	...
	/* GENERATE: upcoming list events*/
		function generate_events_list($args=''){
			...
			$content.=$this->body->get_calendar_header(array(
				...
				'sortbar'=> ((isset($args__['hide_so']) && $args__['hide_so']=='yes')? false: true),
				'date_header'=>false,
		...
		// for a month by default but can change to set time line with args
			public function evo_get_wp_events_array(
				...
				//print_r(count($event_list_array));
		...
		// Apply filters to events lists array
				function _filter_events_list($EL, $args){
					if(!is_array($EL)) return $EL;

					if(isset($args['event_count']) && $args['event_count'] >0){
						$count = (int)$args['event_count'];
		...
		// prefilters for events
			...
			function prefilter_events($eventlist){
				//print_r($eventlist);
				$eventlist = $this->move_ml_yl_to_top($eventlist);
							...
							if( (($only_ft && $_is_featured=='yes') || !$only_ft) && ( $hide_ft && $_is_featured=='no' || !$hide_ft)){
								...
								foreach($repeat_intervals as $interval){
									...
									if(($__year_long_event && !empty($ev_vals['event_year']) && $__event_year==$ev_vals['event_year'][0])
										|| ($__month_long_event && !empty($ev_vals['_event_month']) && $__current_month==$ev_vals['_event_month'][0] && !empty($ev_vals['event_year']) && $__event_year==$ev_vals['event_year'][0] )
										|| ($fe && $me) ){
						...
						if( (($only_ft && $_is_featured=='yes') || !$only_ft) && ( $hide_ft && $_is_featured=='no' || !$hide_ft)){
							...
							if( ( $__year_long_event && !empty($ev_vals['event_year']) && $__current_year==$ev_vals['event_year'][0] )
								|| ($__month_long_event && !empty($ev_vals['_event_month']) && $__current_month==$ev_vals['_event_month'][0] 
									&& !empty($ev_vals['event_year']) && $__current_year==$ev_vals['event_year'][0] )
								|| ($fe && $me )){
		...
		public function generate_event_data(
			...
			// Initial variables
				...
				// schema data
					...
					if($this->__calendar_type =='single' && EVO()->cal->get_prop('evcal_schema_disable_section') =='single' && !$show_schema)
						$show_schema = true;
			...
			foreach($event_list_array as $event_):
				...
				// set how a single event would interact
					...
					// if using bubble eventtop style override ux_val to be lightbox or open in single event page or external link
						if( isset($__shortC_arg['eventtop_style']) && $__shortC_arg['eventtop_style'] == 3 ){
				...
				*/
				...
				// EACH DATA FIELD
					...
					// EVENT LOCATION
						...
						if ( $location_terms && ! is_wp_error( $location_terms ) ){
							...
							// location url
								$location_url = get_term_link($location_terms[0],'event_location');
							// location name
						...
						$_eventcard['timelocation'] = array(
							...
							'location_link_target' => ((!empty($LocTermMeta['evcal_location_link_target']) && $LocTermMeta['evcal_location_link_target']=='yes')?'_blank':''),
					...
					// Event Organizer
						...
						$organizer_term_id = $organizer_name = $organizer_term = false;
						if($organizer_terms && ! is_wp_error( $organizer_terms )){
							...
							$organizer_term = $organizer_terms[0];
						}
				...
				/** CONSTRUCT the EVENT CARD	 **/
					...
					// EVENT LOCATION
						...
						if( $location_name && ($location_address || $lonlat) ){
								...
								$__scheme_data_location = '
									<item style="display:none" itemprop="location" itemscope itemtype="http://schema.org/Place">
										...
										<span itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
											<item itemprop="streetAddress">'. stripslashes($location_address) .'</item>
										</span>';
							...
							// location url
							$event_location_variables .= ($location_url)? ' data-location_url="'.$location_url.'"':null;
				...
				// EVENT TOP
					...
					// date block
						...
						$__eventtop['day_block']= array(
							...
							'show_start_year'=>  	
								(($eventop_fields_ && in_array('eventyear',$eventop_fields) || (isset($__shortC_arg['eventtop_style']) && $__shortC_arg['eventtop_style'] == 3 && $__year_long_event))?'yes':'no'),
				...
				// SCHEME SEO
					...
					// JSON-LD structured data for event
						...
						if($this->__calendar_type =='single' && EVO()->cal->get_prop('evo_remove_jsonld_section') =='single' && !$show_jsonld)
							$show_jsonld = true;

						if( $show_jsonld){
							// description
							$__schema_desc = !empty($event_full_description)? eventon_get_normal_excerpt($event_full_description, 30) : (isset($event->post_title)? '"'.$event->post_title.'"':'');
							...
							'{	"@context": "http://schema.org",
							  	...
							  	"endDate": "'.$DATE_end_val['Y'].'-'.
							  		...
							  		$DATE_end_val['H'].'-'.
							  		$DATE_end_val['i'].'-00",
							  	"image":'.(!empty($img_src) &&!empty($img_src[0])? '"'.$img_src[0].'"':'"'.$event_permalink . '"').',
				...
				// CLASES - attribute
					...
					// TILES STYLE
						if(!empty($__shortC_arg['tiles']) && $__shortC_arg['tiles'] =='yes'){
							...
							// event top style
							if( isset($__shortC_arg['eventtop_style']) && ($__shortC_arg['eventtop_style'] == 1 || $__shortC_arg['eventtop_style'] == 2 || $__shortC_arg['eventtop_style'] == 3 ) ){
								$_eventInAttr['style'][] = 'background-color: '.$event_color.';';
							}else{
								$_eventInAttr['style'][] = 'border-color: '.$event_color.';';
							}	
						}
					...
					// Unique repeating event class name
						$_eventClasses[] = 'event_'.$EVO_Event->ID.'_'.$EVO_Event->ri;

				$_eventAttr['id'] = 'event_'.$EVO_Event->ID;
				$_eventAttr['class'] = $this->helper->implode($_eventClasses);
				$_eventAttr['data-event_id'] = $event_id;
				$_eventAttr['data-ri'] = $EVO_Event->ri.'r';
				...
				$_eventInAttr['data-runjs'] = apply_filters('evo_event_run_json_onclick', false, $EVO_Event);

				// if event is linking to external site
					if($event_ux_val == '2'){
						$_eventInAttr['href'] = $event_permalink;
	...
	/**	 Add other filters to wp_query argument	 */
		public function apply_evo_filters_to_wp_argument($wp_arguments, $arguments){
			...
			if($filters!=false && is_array($filters) ){
				...
				if( sizeof($filters) > 1) $filter_tax['relation']='AND';
