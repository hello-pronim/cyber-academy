	...
	function evo_initial_check($slug='eventon'){
		...
		if($slug=='eventon'){
			...
			if(is_multisite()){
				...
				if(is_plugin_active_for_network('eventon/eventon.php')){
					return true;
				}else{
					$blogs = wp_get_sites();
					foreach($blogs as $blog){
	...
	// @updated 2.7
		function evo_process_to_past_events(){
			...
			while($events->have_posts()): $events->the_post();
				...
				// skip repeat, month long and year long events
				if( $EV->check_yn( 'evcal_repeat') ) continue;
...
// moved to core since 2.3.13
	function eventon_get_repeat_intervals($unix_S, $unix_E){
		...
		$week_repeat_by = (isset($_POST['evp_repeat_rb_wk']))? $_POST['evp_repeat_rb_wk']: 'sing';
			...
			if(!empty($week_repeat_days)) sort($week_repeat_days);
		...
		if($repeat_type=='custom'&& !empty($_POST['repeat_intervals'])){
			...
			for($x =0; $x<=$repeat_count; $x++){
				...
				if($repeat_type == 'monthly' && $month_repeat_by=='dow' && !empty($days) && is_array($days) ){
					...
					// for each day - repeat on selected days		
					foreach($days as $day){
						...
						// week of the month as multiple value
						if( is_array($wom)){
							foreach($wom as $week_om){
								$new_unix_S = (-1 == $week_om) 
							    	? strtotime( "last ".$Names[$day], $NextMonthTS ) 
							    	: strtotime( $Names[$day]." + ".($week_om-1)." weeks", $ThisMonthTS );

							    $new_start_time_repeat = $new_unix_S+$dif_S; // get full start time for repeat
							    if($new_start_time_repeat<$unix_S) continue; // skip old dates

							    $new_end_time_repeat = $new_start_time_repeat+$dif_s_e; // end time for repeat
						...
						$data .= date("Y-m-j", $new_start_time_repeat).' ';	// debuging purpose
...
// SUPPORT FUNCTIONS
	...
	// Generate location latLon from address
		function eventon_get_latlon_from_address($address){
			...
			EVO()->cal->set_cur('evcal_1');
			$gmap_api = EVO()->cal->get_prop('evo_gmap_api_key');
