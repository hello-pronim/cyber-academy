...
class evo_frontend {
	...
	public function __construct(){
		...
		$this->evopt1 = EVO()->evo_generator->evopt1;
	...
	// styles and scripts
		public function register_scripts() {
			...
			wp_register_script('evo_handlebars',plugins_url(EVENTON_BASE) . '/assets/js/handlebars.js', array('jquery'), EVO()->version, true ); // 2.6.8
			
			wp_register_script('evo_mobile',EVO()->assets_path.'js/jquery.mobile.min.js', array('jquery'), EVO()->version, true ); // 2.2.17
			
			wp_register_script('evcal_easing', EVO()->assets_path. 'js/jquery.easing.1.3.js', array('jquery'),'1.0',true );//2.2.24
			wp_register_script('evo_mouse', EVO()->assets_path. 'js/jquery.mousewheel.min.js', array('jquery'),EVO()->version,true );//2.2.24
			...
			//wp_register_style('evo_osmap',EVO()->assets_path.'css/leaflet.css', array(), EVO()->version);	
			...
			// STYLES
			wp_register_style('evo_font_icons',EVO()->assets_path.'fonts/font-awesome.css','',EVO()->version);		
			...
			global $is_IE;
			if ( $is_IE ) {
				wp_register_style( 'ieStyle', EVO()->assets_path.'css/ie.css', array(), '1.0' );
		...
		// ADD SOcial media to event card
			function add_social_media_to_eventcard($object, $helpers){
				...
				if( (!empty($evo_opt['evosm_som']) && $evo_opt['evosm_som']=='yes' && $__calendar_type=='single') 
					...
					$social_sites = apply_filters('evo_se_social_media', array(
						...
						'FacebookShare'    => array(
							...
							'url' => '<a class=" evo_ss" target="_blank" onclick="'.$fb_js.'"
								href="http://www.facebook.com/sharer.php?u=PERMALINK" title="'.evo_lang('Share on facebook').'"><i class="fa fa-facebook"></i></a>',
						),
						'Twitter'    => array(
							'key'=>'eventonsm_tw',
							'counter' =>1,
							'favicon' => 'twitter.png',
							'url' => '<a class="tw evo_ss" onclick="'.$tw_js.'" href="http://twitter.com/intent/tweet?text=TITLECOUNT&#32;-&#32;&url=PERMALINK" title="'.evo_lang('Share on Twitter').'" rel="nofollow" target="_blank" data-url="PERMALINK"><i class="fa fa-twitter"></i></a>',
						),
						'LinkedIn'=> array(
							'key'=>'eventonsm_ln',
							'counter'=>1,'favicon' => 'linkedin.png',
							'url' => '<a class="li evo_ss" href="http://www.linkedin.com/shareArticle?mini=true&url=PERMALINK&title=TITLE&summary=SUMMARY" target="_blank" title="'.evo_lang('Share on Linkedin').'"><i class="fa fa-linkedin"></i></a>',
						),						
						'Pinterest' => Array (
							'key'=>'eventonsm_pn',
							'counter' =>1,
					...
					if($sm_count>0){
						$O= "<div class='bordb evo_metarow_socialmedia evcal_evdata_row ".$helpers['end_row_class']."'>".$output_sm."<div class='clear'></div>";
						$O .= $helpers['end'];
						$O .= "</div>";
						return $O;
					}
