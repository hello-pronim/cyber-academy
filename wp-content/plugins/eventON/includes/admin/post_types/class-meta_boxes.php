...
class evo_event_metaboxes{
	...
	public function __construct(){
		...
		add_filter( 'admin_post_thumbnail_html', array($this,'add_event_images' ), 10, 3);
	}

	// additions to the event image section
	function add_event_images($c, $event_id, $thumbnail_id){
		$after = '';
		...
		if(!$thumbnail_id){
			...
			// compatibility with old event photos addon
				$evoep_imgs = get_post_meta( $event_id, 'evoep_images', true);
		...
		function ajde_events_settings_per_post(){
			...
			?>
				...
				<div class='evo_event_opts'>
					...
					<p class='yesno_row evo'>
						<?php 							
						echo apply_filters('evo_eventedit_admin_cancel_btn', $ajde->wp_admin->html_yesnobtn(
							array(
								'id'=>'_cancel', 
	...
	// MAIN META BOX CONTENT
		...
		?>	
			...
			<?php	endforeach;	?>
					<div class='evomb_section additinoal_functionality' id='<?php echo $mBOX['id'];?>'>			
						<div class='evomb_header'>
			...
			// if Repeating event save repeating intervals
				if( eventon_is_good_repeat_data()  ){
					...
					if(!empty($proper_time['unix_start'])){
						...
						// save the events date range
							$this->EVENT->set_prop('date_range_start', $proper_time['unix_start']);
							$this->EVENT->set_prop('date_range_end',$end_range);
					}
				}else{
					// save the events date range
						$this->EVENT->set_prop('date_range_start', $proper_time['unix_start']);
						$this->EVENT->set_prop('date_range_end',$proper_time['unix_end']);
				}
