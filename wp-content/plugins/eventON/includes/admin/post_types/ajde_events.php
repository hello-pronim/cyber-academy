		...
		function eventon_custom_event_columns( $column , $post_id) {
			...
			switch ($column) {
				...
				case "name" :
					...
					// if event cancelled
					if( $EVENT->is_cancelled()) echo "<span style='display:block;text-transform:uppercase;font-size:11px'>".__('Cancelled','eventon')."</span>";
					...
					$event = EVO()->evo_event_item->get_event($post->ID);
					
					//print_r($event);
					
					/* Custom inline data for eventon */
					echo '<div class="hidden" id="eventon_inline_' . $post->ID . '">';
					foreach( EVO()->evo_event_item->get_event_fields_edit()  as $field){
	...
	// Custom quick edit - form
		...
		private function quick_edit_save($post_id, $EVENT){
			...
			foreach( apply_filters('eventon_quick_save_fields', array(
				...
				$EVENT->set_prop( $field, eventon_clean($_REQUEST[ $field ])  );
			}
