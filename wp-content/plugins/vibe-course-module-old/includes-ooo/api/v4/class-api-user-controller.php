...
if ( ! class_exists( 'BP_Course_New_Rest_User_Controller' ) ) {
	...
	class BP_Course_New_Rest_User_Controller extends BP_Course_New_Rest_Controller {
		...
		function get_course_status($request){
			...
			if(class_exists('WPLMS_tips')){
			    ...
			    if(isset($tips) && isset($tips->settings)){
			          $finish_course_auto_trigger = $tips->settings['finish_course_auto_trigger'];
			    }
			}
				...
				if(!empty($package)){
		            ...
		            if(!empty($package)){
		                if(!empty($package['type'])){
		                    $pack =  do_shortcode('[iframe package_type="'.$package['type'].'" no_script="1"]'.$package['path'].'[/iframe]');
		                }else{
		                    $pack = do_shortcode('[iframe]'.$package['path'].'[/iframe]');
		                }
		            }
					$return = array('status'=>1,'package' => $pack);
				}else{
					...
					if(is_numeric($item)){
						...
						if(bp_course_get_post_type($item) == 'unit'){
							...
							$curriculum_arr[] = apply_filters('bp_course_api_course_curriculum_unit',array(
								...
								'content'   => '',
								'status'    => $complete,
								'icon'		=> wplms_get_element_icon(wplms_get_element_type($item,'unit')),
								'meta'		=> array()
							));
		...
		function get_course_status_item($request,$bypassed=false){
			...
			$return = array('title'=>$item->post_title);
			$meta=array('access'=>0);
		...
		function mark_unit_complete($item_id,$user_id,$course_id,$progress=null){
			...
			if(!empty($user_id) && !empty($item_id)){
				...
				$data = array('status'=>true,'icon'=>wplms_get_element_icon(wplms_get_element_type($item_id,'unit')),'message'=>sprintf(_x('Unit %s  completed in Course %s','','wplms'),$unit_title,$course_title));
			}
			return $data;
		}
		...
		function add_user_result($request){
			...
			if(!empty($qstatus) && $qstatus > 2){
				...
				$c_count = count($course_curriculum);
	      		if($c_count){
					$progress = $progress + round((100/$c_count),2);
	      		}
				if($progress > 100){$progress = 100;}
