...
if ( ! class_exists( 'BP_Course_Rest_Course_Controller' ) ) {
	...
	class BP_Course_Rest_User_Controller extends BP_Course_Rest_Controller {
		...
		public function register_routes() {
			...
			register_rest_route( $this->namespace, '/'. $this->type .'/unitcomments/(?P<unit>\d+)/(?P<type>(new|edit|reply))/(?P<commentID>\d+)/', array(
				'methods'                   =>   'POST',
		...
		public function get_user_permissions_check($request){
			$user = apply_filters('wplms_api_permission','',$request);
		...
		function get_course_status($request){
			...
			$section_duration = 0;
			$unit_id = wplms_plugin_get_course_unfinished_unit($course_id);
			$curr_unit_key = 0;
			$i = 0;
			$first_curriculum_item_key = 0;
			...
			foreach($curriculum as $key => $item){
				if(is_numeric($item)){
					...
					if(empty($first_curriculum_item_key))
					...
					if(bp_course_get_post_type($item) == 'unit'){
						...
						$apost = get_post($item);
						$drip_check = bp_course_get_drip_status($course_id,$user_id,$item);
						$assign = apply_filters('bp_course_api_course_curriculum_quiz',array(
							'key'		=> $i,
							'id'		=> $item,
							'type'		=> 'wplms-assignment',
							'title'		=> get_the_title($item),
							'duration'	=> $data['duration'],
							'content'   => do_shortcode($apost->post_content),
							'status'    => $data['assignment_status'],
							'meta'		=> $data,
							
						));
						if($drip_check['status']){
							$assign['meta']['access']    = 0;
						}else{
							$assign['meta']['access']    = 1;
						}
						$curriculum_arr[] = $assign;
		...
		function get_course_status_item($request){
			...
			//print_r('$$'.$course_id .' - '.$user_id.' = '.bp_course_is_member($course_id,$user_id));
		...
		function get_course_status_item_complete($request){
			...
			if(!empty($user_id) && !empty($item_id)){
				...
				bp_course_update_user_progress($user_id,$course_id,$progress);
				$meta['progress']=$progress;
				$data = array('status'=>true,'message'=>_x('Unit marked complete','','wplms'));
			}
			return new WP_REST_Response( $data, 200 );
		}
		...
		function get_single_quiz_data($request){
			...
			$data = apply_filters( 'bp_course_api_get_user_single_quiz_data',$quiz_data, $request );
			return new WP_REST_Response( $data, 200 );
		}
		...
		function add_user_result($request){
			...
			$stop_progress = apply_filters('bp_course_stop_course_progress',true,$course_id);
	      	...
	      	$next_unit = null;
	      	$flag = apply_filters('wplms_next_unit_access',true,$post->quiz_id);
	      	if(is_numeric($course_id) && $stop_progress && $flag){
	        	...
	        	if($key <=(count($curriculum)-1) ){  // Check if not the last unit
	          		$key++;
	          		if(isset($curriculum[$key])){
	          			$next_unit =  $curriculum[$key].'##';
	          		}
	        	}
	      	}
				...
				if(is_array($post->results)){
					foreach($post->results as $res){
						...
						if(!empty($res->id)){
							...
							$correct_data[$res->id] = round(($correct_count/($incorrect_count+$correct_count))*100);
...
$this->user_id = 1;//remove it later please
		...
		function start_quiz(){
			...
			if(is_numeric($post->quiz_id)){
				$quiz_duration_parameter = apply_filters('vibe_quiz_duration_parameter',60,$post->quiz_id);
              	$quiz_duration = get_post_meta($post->quiz_id,'vibe_duration',true) * $quiz_duration_parameter; // Quiz duration in seconds
		...
		*/
		...
		function get_unit_comments($request){
			...
			$comments = get_comments(apply_filters('wplms_api_get_unit_comments_args',array(
	          'post_id' => $unit_id,
	          'status'=>'approve',
	          'number'=>$per_page,
	          'offset'=>$offset,
       ...
       function get_comment_child($comment_id)
       {
		 ...
		 function add_edit_new_unit_comments($request){
			...
			switch($type){
			    ...
			    case 'edit':
						...
						if($this->user_id==$old_comment_user_id){
							$commentarr = array();
		...
		function get_assignment_data($assignment_id){
			...
			if(!empty($assignment_id)){
		        ...
		        if(null==$assignment_start_time){
	            	...
	            	$args = array(
			            'status' => 'approve',
			            'user_id' => $this->user_id, // use user_id
				...
				$data=array(
					...
					'flag'=>$flag,
					'marks'=>(int)get_post_meta($assignment_id,$this->user_id,true),
		...
		function uploadAssignments($request){    
		    ...
		    /**  
			* @param  $_POST['comment'] gives comment _content
			* @param $_FILES['file'] gives file
			**/
			$post = json_decode(file_get_contents('php://input'));
            $assignment_id = $request->get_param('assignment_id');
           	$body =json_decode(stripslashes($_POST['body']),true);
			$user_data= $this->fetch_user($this->user_id);
            
			$attachment_ids = [];

			/* check for expired or not or started
		    if(0==get_post_meta($assignment_id,$this->user_id,true)){
            	
            	$vibe_assignment_duration=get_post_meta($assignment_id,'vibe_assignment_duration',true);
				$vibe_assignment_duration_parameter=get_post_meta($assignment_id,'vibe_assignment_duration_parameter',true);
				$duration=$vibe_assignment_duration*$vibe_assignment_duration_parameter;
				$start=get_user_meta($this->user_id,$assignment_id,true);
           ...
           **/
          	if(!empty($body['attachments'])){
          		foreach($body['attachments'] as $key=>$meta){
          			$__file = $_FILES['files_'.$key];
          			$attachment_id = $this->add_attachment_to_assignment($assignment_id,$__file,1 );
          			if($attachment_id){
          				...
          				$data['attachment_urls'][]=array('url'=>$attachment_url,'name'=>get_the_title($attachment_id));
          			}
		...
		function contentAssignment($request){
				...
				if(is_array($attachment_Ids)){
					...
					{
						$attachment_url[]= wp_get_attachment_url($attachment_Id);
						$attachment_data[] = array('url'=>wp_get_attachment_url($attachment_Id),'name'=>get_the_title($attachment_Id));     
					}
		         }else{
		         	$attachment_url=$attachment_Ids;
		         }   
	            ...
	            if(null==$umeta){
	            	...
	            	$data['marks']=0;
	            	$data['duration']=$vibe_assignment_duration*$vibe_assignment_duration_parameter;
	            	$data['flag']=0;   
	            }
	            elseif(0==$umeta){
	            	...
	            	$data['remaining'] =  ($start_time+$data['duration']) - time();
	            	if($data['remaining'] <= 0){
	            		$data['remaining'] = 0;
	            	}
	            }else{
	    ...
	    function AssignmentResult($request){
	      ...
	      /*  global $wpdb,$bp;
	        ...
	        if(null==get_post_meta($assignment_id,$this->user_id,true)){
	        	$data['message']=_x('Assignment not started.','API message','wplms');
	        	$data['marks']=0;
	        	$data['total']= get_post_meta($assignment_id,'vibe_assignment_marks',true);
	        }
	        elseif(0==get_post_meta($assignment_id,$this->user_id,true)){
	...
	function get_forums($request){
	    ...
	    function get_replies($request){
	    	...
	    	if(function_exists('bbp_has_replies')){
		    	...
		    	if ( bbp_has_replies($args) ) :
				 	while ( bbp_replies() ) : bbp_the_reply();
						...
						$reply_post = get_post($id);
						
						$is_private = get_post_meta($reply_id,'_bbp_reply_is_private');
						if(!empty($is_private) && ($user_id != $reply_post->post_author) && !user_can($user_id,'edit_posts') ){
							$reply_post->post_content = _x('Reply is set to private!','Reply is set to private!','wplms');
						}
				...
				if(!empty($replies)){
					foreach ($replies as $key => $value) {
						$user_id = (int)$replies[$key]->post_author;
						$replies[$key]->user = $this->get_user_by_ID($user_id);
					}
					$status = 1;
					$message = _x('Replies found!','Replies found!','wplms');
				}else{
					$status = 0;
					$message = _x('Replies not found!','Replies not found!','wplms');
				}
	    	...
	    	$data = apply_filters('vibe_get_replies_api',$data,$args);
	    	return new WP_REST_Response($data, 200); 
	    }	
		...
		function create_forum($request){
			...
			// Create Edit Delete Update restiction start
			$data = array(
				'status' => 0,
    			'data' => [],
    			'message' => _x('Create Edit Delete Update functionality not enabled from site','BB-Press Plugin not active!','wplms')
			);
			return new WP_REST_Response($data, 200);
			// Create Edit Delete Update restiction end
	    	...
	    	if(!empty($user_id)){
		    	...
		    	if(function_exists('bbp_insert_forum')){
			   		...
			   		$flag = bbp_insert_forum( $args, $forum_meta );

					// set message from topics
					if(!empty($flag)){
						$status = 1;
						$message = _x('Forum Created','Forum Created','wplms');
					}else{
						$status = 0;
						$message = _x('Forum not Created','Forum not Created','wplms');
					}
    			...
    			$data=array(
	    			'status' => 0,
	    			'data' => [],
	    			'message' => _x('Authorization error!','Authorization error!','wplms')
	    		);
	    	}


	    	$data = apply_filters('vibe_create_forum_api',$data,$args);
	    	return new WP_REST_Response($data, 200); 
		}
		...
		function create_topic($request){
	    	...
	    	if(!empty($user_id)){
		    	...
		    	if(function_exists('bbp_insert_topic') ){
		    		if(!empty($topic_data['post_content'])&& !empty($topic_data['post_title']) && !empty($topic_data['post_parent']) && !empty($topic_data['post_author']) && !empty($topic_meta['forum_id'])){
						...
						if(!empty($flag)){
							$status = 1;
							$message = _x('Topic Created','Topic Created','wplms');
						}else{
							$status = 0;
							$message = _x('Topic not Created','Topic not Created','wplms');
						}
		    			...
		    			$data=array(
			    			'status' => 0,
			    			'data' => [],
			    			'message' => _x('Passing Arguments not valid!','Passing Arguments not valid!','wplms')
	    				);
		    		} 
		    	}else{
		    		$data=array(
	    	...
	    	$data = apply_filters('vibe_create_topic_api',$data,$args);
	    	return new WP_REST_Response($data, 200); 
		} 	
		...
		function create_reply($request){
	    	...
	    	if(!empty($user_id)){
	    		if( !empty($reply_meta['forum_id']) && !empty($reply_meta['topic_id']) && !empty($reply_data['post_content']) && !empty($reply_data['post_parent']) && !empty($reply_data['post_author']) ){
	    			if(function_exists('bbp_insert_reply')){
						...
						if(!empty($reply_id)){
							$status = 1;
							$data = bbp_get_reply( $reply_id );
								$user_id = (int)$data->post_author;
								$data->user = $this->get_user_by_ID($user_id);
							$message = _x('Reply Created','Reply Created','wplms');
						}else{
							$status = 0;
							$data = false;
							$message = _x('Reply not Created','Reply not Created','wplms');
						}
	    	...
	    	$data = apply_filters('vibe_create_reply_api',$data,$args);
	    	return new WP_REST_Response($data, 200); 
		} 	
		...
		function update_reply($request){
			...
			$user_id = (int)$this->user_id;
			/* validate here user to update reply..  */
			...
			if(!empty($reply_id)){
								...
								if(($user_id == $reply->post_author) || user_can($user_id,'edit_post') ){
									...
									if(!empty($flag)){
										...
										$data=array(
							    			'status' => 1,
							    			'data' => $ereply,
							    			'message' => _x('Reply Updated.','Reply Updated','wplms')
							    		);
									}else{
										$data=array(
							    			'status' => 0,
							    			'data' => false,
							    			'message' => _x('Reply not Updated.','Reply not Updated','wplms')
							    		);
									}
								}else{
									$data=array(
						    			'status' => 0,
						    			'data' => false,
						    			'message' =>  _x('You are not a valid user to update this reply','Not success','wplms')
					    			);
								}	
							}else{
								$data=array(
					    			'status' => 0,
					    			'data' => false,
					    			'message' => _x('Reply not Exist.','Reply not Exist','wplms')
					    		);
							}
			    		}else{
			    			$data=array(
				...
				$data=array(
	    			'status' => 0,
	    			'data' => false,
	    			'message' => _x('Insufficient data','Insufficient data','wplms')
	    		);
			}
			$data = apply_filters('vibe_update_reply_api',$data,$args);
	    	return new WP_REST_Response($data, 200); 
		}
		...
		function delete_reply($request){
					...
					if ( empty( $reply ) ){
						...
						if($user_id == $reply->post_author){
							switch ( $sub_action ) {
								...
								if($success){
									$message = _x('Reply trash successfull','Reply trash successfull','wplms');
								}
								break;
							...
							case 'untrash':
								...
								if($success){
									$message = _x('Reply untrash successfull','Reply untrash successfull','wplms');
								}
								break;
							...
							case 'delete':
								...
								if($success){
									$message = _x('Reply delete successfull','Reply delete successfull','wplms');
								}
								break;
							}
							...
							if($success){
								...
								$data=array(
					    			'status' => 0,
					    			'data' => $success,
					    			'message' =>  _x('Not success','Not success','wplms')
				    			);
							}
						}else{
							$data=array(
				    			'status' => 0,
				    			'data' => false,
				    			'message' =>  _x('You are not a valid user to delete this reply','Not success','wplms')
			    			);
						}	
					}
		    	}else{
			...
			$data = apply_filters('vibe_delete_reply_api',$data,$args);
	    	return new WP_REST_Response($data, 200); 
		}

		function get_my_topics($user_id,$per_view,$paged){
	    	if(!empty($user_id)){
	    		$user_id = (int)$user_id;
		    	if(function_exists('bbp_has_topics')){
		    		//set args here
		    		$args = array(
		    			'posts_per_page' => $per_view,
		    			'paged' => $paged,
		    			'order' => "DESC",
						'author' => $user_id
		    		);
			...
			$data = apply_filters('vibe_my_topics_api',$data,$args);
	    	return new WP_REST_Response($data, 200); 
		}


		function get_my_replies($user_id,$per_view,$paged){
	    	if(!empty($user_id)){
	    		$user_id = (int)$user_id;
		    	if(function_exists('bbp_has_replies')){
		    		//set args here
			...
			$data = apply_filters('vibe_my_replies_api',$data,$args);
	    	return new WP_REST_Response($data, 200); 
		}
