...
Class BP_Course_Template{
	...
	function get_course_meta($course_id){
		...
		$meta .= '<strong>( '.(empty($count)?'0':$count).' '.__('REVIEWS','wplms').' )</strong> ';
		...
		$meta .='<div class="students"> '.$students.' '.__('STUDENTS','wplms').'</div>';
	...
	*/
	...
	function get_course_credits($args){
		...
		if(vibe_validate($free_course)){
			$credits[] = '<strong>'.apply_filters('wplms_free_course_price',__('FREE','wplms')).'</strong>';
		}else if(vibe_validate($apply_course)){
			$credits[] = '<strong>'.apply_filters('wplms_course_application_label',__('Apply to enroll','wplms')).'</strong>';
		}else{
		...
		if(count($credits) > 1 ){
			...
			$credits_html .= '<div class="pricing_course">
    								<div class="result"><span>'.__('Price Options +','wplms').'</span></div>
								    <div class="drop">';
								    $first = 1;
	...
	function count_pursuing_students($course_id){
		...
		if(empty($number)){
			$number = $wpdb->get_var( apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT count(user_id) as number FROM {$wpdb->usermeta} WHERE meta_key = %s",'course_status'.$course_id)));
		}
	...
	function get_total_course_drip_duration($user_id,$course_id){
		...
		if(empty($total_duration)){
        	...
        	$this->total_drip[$course_id.'_'.$user_id] = intval($drip_duration)*intval($drip_duration_parameter);
		}
		return $this->total_drip[$course_id.'_'.$user_id];
	}	
...
function bp_course_has_items( $args = '' ) {
	...
	if ( empty( $items_template ) ){
		...
		//Object Caching cases are handled by our reliable code
		if(!empty($init->course_id)){ 
			$r['id']=$init->course_id;
		}elseif(is_singular('course') || (isset($post) && $post->post_type == 'course')){
...
if(!function_exists('bp_course_item_view')){
   function bp_course_item_view(){
	...
	function bp_course_get_students_count($course_id = NULL){
		...
		global $wpdb;
		$count = $wpdb->get_var(apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT count(user_id) FROM {$wpdb->usermeta} WHERE meta_key  = %s ",'course_status'.$course_id)));
	...
	function bp_course_get_pagination_count() {
		...
		$pagination_count = sprintf( __( 'Viewing page %1$s of %2$s', 'wplms' ), $items_template->query->query_vars['paged'], $items_template->query->max_num_pages );
...
function wplms_check_private_course($label){ 
	if(strlen($label) < 2){
		return '<strong>'.apply_filters('wplms_private_course_label',__('PRIVATE','wplms')).'</strong>';
	}
	return $label;
}
/**
...
function bp_course_get_course_students($course_id = null,$page_num = null,$loop_number = null){
	global $wpdb,$post;
		...
		$page_num=0;
		$cquery=apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT SQL_CALC_FOUND_ROWS DISTINCT user_id FROM {$wpdb->usermeta} WHERE meta_key = %s LIMIT %d, %d",'course_status'.$course_id,$page_num,$loop_number));
		$course_meta = $wpdb->get_results( $cquery, ARRAY_A);
...
function bp_course_paginate_students_undertaking($course_id=NULL){
	...
	$page_num = 0;
	$query=$wpdb->prepare(apply_filters('wplms_usermeta_direct_query',"SELECT count(user_id) FROM {$wpdb->usermeta} WHERE meta_key = %s ORDER BY meta_value ASC LIMIT %d, %d",'course_status'.$course_id,$page_num,$loop_number));

	if(isset($_GET['status']) && is_numeric($_GET['status'])){
		$query=$wpdb->prepare(apply_filters('wplms_usermeta_direct_query',"SELECT count(user_id) FROM {$wpdb->usermeta} WHERE meta_key = %s AND meta_value = %d ORDER BY meta_value ASC LIMIT %d, %d",'course_status'.$course_id,$_GET['status'],$page_num,$loop_number));
	}
	...
	$return  =	'<div class="pagination"><div><div class="pag-count" id="course-member-count">'.sprintf(__('Viewing page %d of %d ','wplms'),((isset($_GET['items_page']) && $_GET['items_page'])?$_GET['items_page']:1 ),$max_page).'</div>
...
function bp_course_get_question_details($question_id,$force = null){
	...
	$new_content = str_replace('[fillblank]', '______', $content);
	$question['original_content'] = apply_filters('the_content',$new_content );
	...
	if( has_shortcode( $content, 'match' ) ) {
		...
		if(!empty($content_array) && !empty($content_array)){
			...
			preg_match_all("/<li>(.*?)<\/li>/", $content_array[0][2], $s_items);
			if(!empty($s_items)){
				foreach($s_items[1] as $i=>$item){
					$shortcode_items[$i] = do_shortcode($item);
				}
				$match_Array = array(
					'statement' => $content_array[0][1],
					'match' => $shortcode_items,
					'end' => $content_array[0][3],
				);	
				$question['content'] = $match_Array;
				$question['extra_content'] = $match_Array;
			}
		...
		if(!empty($content_array) ){
			...
			$question['extra_content'] = $content_array;
		}else{
			$question['content'] = $content;
			$question['extra_content'] = $content;
		}

	} else if( has_shortcode( $content, 'select' ) ) {
		$content_array = preg_split("/\[select.*?\]/", $content);
		if(!empty($content_array) ){
			foreach($content_array as $k=>$c){
				$content_array[$k] = apply_filters('the_content',$c);
			}
	...
	$options_with_the_content =array();
	if(!empty($question['options'])){
		foreach ($question['options'] as $key => $op) {
			$options_with_the_content[]= do_shortcode($op);
		}
	}
	
	$question['options'] = $options_with_the_content;
	if(in_array($question['type'], array('sort','match')) && !empty($question['options'])){
...
function bp_course_get_user_question_marks($quiz_id,$question_id,$user_id){
	$template = BP_Course_Template::init();
	return apply_filters('bp_course_get_user_question_marks',$template->get_answer_marks($quiz_id,$question_id,$user_id),$quiz_id,$question_id,$user_id);
}
...
function bp_course_generate_user_result($quiz_id,$user_id,$user_result,$activity_id){
	...
	if(empty($activity_id))
		return;
	if(!is_numeric($activity_id)){
		update_user_meta($user_id,'manual_intermediate_results'.$quiz_id,$user_result);
	}else{


		$custom = apply_filters('wplms_custom_bp_course_generate_user_result',false,$quiz_id,$user_id,$user_result,$activity_id);

		if($custom){
			vibe_bp_record_activity_meta($activity_id,'quiz_results',$user_result);
		}else{
...
function vibe_bp_record_activity_meta($activity_id,$meta_key,$meta_value){
	global $wpdb,$bp;
	
	$meta_value = maybe_serialize( $meta_value);
	$meta_id = $wpdb->get_var($wpdb->prepare(
        "SELECT id FROM {$bp->activity->table_name_meta} WHERE meta_key = %s AND activity_id = %d ORDER BY id DESC LIMIT 0,1",
        $meta_key,
        $activity_id
    ));
    if(!empty($meta_id)){

    	$flag = $wpdb->query($wpdb->prepare("UPDATE {$bp->activity->table_name_meta} SET meta_value = %s WHERE id = %d",$meta_value,$meta_id));
    }else{

    	$flag = $wpdb->query($wpdb->prepare("INSERT INTO {$bp->activity->table_name_meta} (activity_id, meta_key, meta_value) VALUES (%d, %s, %s)",$activity_id,$meta_key,$meta_value));
...
function bp_course_get_user_unit_completion_time($user_id,$unit_id,$course_id=null){
  ...
  if(!empty($course_id)){
  	...
  	return apply_filters('bp_course_get_user_unit_completion_time',$time,$unit_id,$user_id,$course_id);
}
...
function bp_course_check_quiz_complete($quiz_id=NULL,$user_id=NULL,$course_id=NULL){
	...
	if(isset($quiz_check) && $quiz_check !='')
		...
		$bool =  false;
	return apply_filters('bp_course_check_quiz_complete',$bool,$quiz_id,$user_id,$course_id);
}
...
function bp_course_get_instructor_courses($user_id){
	global $wpdb;
	$courses= $wpdb->get_results(apply_filters('wplms_dashboard_courses_instructors',
                $wpdb->prepare("SELECT ID as value,post_title as label From {$wpdb->posts} WHERE post_status='publish' AND post_type ='course' AND post_author = %d",
                    $user_id
                ),$user_id),ARRAY_A);
	return $courses;
}
...
function bp_course_get_user_courses($user_id,$status = NULL){
  	...
  	if(function_exists('bp_is_active') && bp_is_active('activity')){
  		if(!empty($status) && in_array($status,array('active','expired'))){
			$query = $wpdb->get_results(apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("
  		...
  		}else if(!empty($status) && in_array($status,array('subscribe_course','start_course','submit_course','course_evaluated'))){
  			$query = $wpdb->get_results(apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("
...
function bp_course_get_user_statuses(){
	return apply_filters('bp_course_user_statuses',array(
      1=>_x('Start Course','course button status','wplms'),
      2=>_x('Continue Course','course button status','wplms'),
      3=>_x('Course under Evaluation','course button status','wplms'),
      4=>_x('Course Finished','course button status','wplms')
    ));
}
...
function is_instructor($id=NULL,$user_id=NULL){
...
function bp_get_course_check_course_complete($args=NULL){ // AUTO EVALUATION FOR COURSE
	...
	$return_array = array('status'=>0,'message'=>$m);
	...
	if(isset($course_curriculum) && count($course_curriculum)){
		...
		if(!$flag){
			...
			if(vibe_validate($auto_eval)){
				...
				$return .='<div class="message updated"><p>'.__('COURSE EVALUATED ','wplms').'</p></div>';
				...
				bp_course_update_user_course_status($user_id,$id,4);
				//end finish bit
			    ...
			    if($badge_filter){  
            		...
            		if(isset($badge) && is_numeric($b)){
			        	...
			        	$return .='<div class="congrats_badge">'.__('Congratulations ! You\'ve earned the ','wplms').' <strong>'.$b_title.'</strong> '.__('Badge','wplms').'<a class="tip ajax-badge" data-course="'.get_the_title($id).'" title="'.get_post_meta($id,'vibe_course_badge_title',true).'"><img src="'.$badge_url[0].'" title="'.$badge['title'].'"/></a></div>';
            		}
				...
				bp_course_update_user_course_status($user_id,$id,3);// 3 determines Course is Complete
				do_action('wplms_submit_course',$id,$user_id);
			}
			...
			$message = __('PLEASE COMPLETE THE ','wplms').$type.' : '.get_the_title($flag);
		...
		$retun .=__('COURSE CURRICULUM NOT SET','wplms');
	}	
	$return .='</div>';
	if($array){
		return $return_array;
            ...
            if(!isset($pre_unit_time) || $pre_unit_time ==''){
                ...
                if(!empty($pre_unit_time)){
                     ...
                     if($value > time()){
                            ...
                            $return['status'] = 1;
                        	$return['message'] = '<div class="message"><p>'.sprintf(__('%s will be available in %s','wplms'),$element,tofriendlytime($value-time())).'</p></div>';
                        ...
                        if(isset($pre_unit_key)){
                          ...
                          if(!empty($completed)){
                              ...
                              $element = apply_filters('wplms_drip_feed_element_in_message',__('Unit','wplms'),$course_id);
                                ...
                                $return['status'] = 1;
                        		$return['message'] =  '<div class="message"><p>'.sprintf(__('%s can not be accessed.','wplms'),$element).'</p></div>';
...
function bp_course_get_course_applicants_count($course_id){
	...
	$count = $wpdb->get_var(apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT count(user_id) FROM {$wpdb->usermeta} WHERE meta_key = %s AND meta_value = %d",'apply_course'.$course_id,$course_id)));
	...
	if(bp_is_active('activity')){
		if(!empty($activity_id)){
			...
			$result = $wpdb->get_var($wpdb->prepare("
				...
				AND a.secondary_item_id = %d
				AND a.user_id = %d
				ORDER BY a.id LIMIT 0,1",$quiz_id ,$user_id));
		}
	}

	return apply_filters('bp_course_get_quiz_results_meta',$result,$quiz_id,$user_id,$activity_id );
}
...
function bp_course_quiz_results($quiz_id,$user_id,$course=NULL){
	...
	if(isset($course) && is_numeric($course) && !defined('DOING_AJAX')){
		$extra = '<a href="'.get_permalink($course).'" class="small_link">( &larr; '.__('BACK TO COURSE','wplms').' )</a>';	
	}
			...
			if(!empty($question['content'])){
				...
				echo '<li>
					...
					if($question['type'] == 'survey'){
						echo '<strong>'.__('Marked Choice :','wplms').'</strong> '.apply_filters('question_the_content',$question['marked_answer']).'<span>'._x('Score','survery question score awarded','wplms').' : '.$question['marks'].'</span>';
						if(!empty($question['explaination']) && strlen($question['explaination']) > 2){
					 		echo '<a class="show_explaination tip" title="'.__('View explanation','wplms').'"></a>';
						}
						echo '</strong>';
						...
						echo '<strong>'.__('Marked Answer :','wplms').'</strong> '.apply_filters('question_the_content',$question['marked_answer']);
						$show_correct_answer_explanation = apply_filters('wplms_show_correct_answer_explanation_unattempted',(isset($question['marks']) && $question['marks'] !=''),$question_id,$quiz_id);
						if(isset($question['correct_answer']) && $question['correct_answer'] !='' && $show_correct_answer_explanation && $flag){
							echo '<strong>'.__('Correct Answer : ','wplms').do_shortcode($question['correct_answer']).'<span>';
						 	if(!empty($question['explaination']) && strlen($question['explaination']) > 5){
						 		echo '<a class="show_explaination tip" title="'.__('View answer explanation','wplms').'"></a>';
							}
							echo '</span></strong>';
						}

						echo '<span> '.__('Total Marks :','wplms').' '.apply_filters('wplms_quiz_results_quesstion_total_marks',$question['max_marks'],$quiz_id,$question,$user_id).'</span>';
						$total_sum += $question['max_marks'];


						if(isset($question['marks']) ){
							if($question['marks'] > 0){
								echo '<span>'.__('MARKS OBTAINED','wplms').' <i class="icon-check"></i> '.apply_filters('wplms_quiz_results_quesstion_user_marks',$question['marks'],$quiz_id,$question,$user_id).'</span>';
							}else{
								echo '<span>'.__('MARKS OBTAINED','wplms').' <i class="icon-x"></i> '.apply_filters('wplms_quiz_results_quesstion_user_marks',$question['marks'],$quiz_id,$question,$user_id).'</span>';
							}
							$sum += intval($question['marks']);
						}else{
							echo '<span>'.__('Marks Obtained','wplms').' <i class="icon-alarm"></i></span>';
						}
			...
			foreach($questions['ques'] as $key=>$question){
				if(isset($question) && is_numeric($question)){
					...
					if($type == 'survey'){
						...
						echo '<strong>'.__('Marked Choice :','wplms').'</strong> '.apply_filters('the_content',$user_marked_answer).'<span>'._x('Score','survery question score awarded','wplms').' : '.$marks.'</span>';
						if(!empty($question['explaination']) && strlen($question['explaination']) > 5){
					    ...
					    switch($type){
			            	...
			            	break;
					      	case 'fillblank':
					      		...
					      		//CODE CHANGED BY MANISH (TRELLO BUG : Multiple 'and' in case of empty field)					      		
								$arr = explode("|",$user_marked_answer);
								$h = Array();
								...
								$user_marked_answer = implode("|",$h);

								//MANISH CODES ENDS HERE;								
						...
						$show_correct_answer_explanation = apply_filters('wplms_show_correct_answer_explanation_unattempted',(isset($marks) && $marks !=''),$question,$quiz_id);
						if(isset($correct_answer) && $correct_answer !='' && $show_correct_answer_explanation && $flag){
							$explaination = get_post_meta($question,'vibe_question_explaination',true);
							$user_result[$question]['explaination'] = apply_filters('the_content',$explaination);
							echo (($type != 'survey')?'<strong>'.__('Correct Answer :','wplms').'<span>'.apply_filters('the_content',$ans).' ':'<strong><span>');
							echo ((isset($explaination) && $explaination && strlen($explaination) > 5)?'<a class="show_explaination tip" title="'.__('View answer explanation','wplms').'"></a>':'').'</span></strong>';
						}

						$total_sum = $total_sum+intval($questions['marks'][$key]);
						$user_result[$question]['max_marks'] = $questions['marks'][$key];
						echo '<span> '.__('Total Marks :','wplms').' '.apply_filters('wplms_quiz_results_quesstion_total_marks',$questions['marks'][$key],$quiz_id,$question,$user_id).'</span>';

						if(isset($marks) && $marks !=''){
							if($marks > 0){
								echo '<span>'.__('MARKS OBTAINED','wplms').' <i class="icon-check"></i> '.$marks.'</span>';
							}else{
								echo '<span>'.__('MARKS OBTAINED','wplms').' <i class="icon-x"></i> '.$marks.'</span>';
							}
							$user_result[$question]['marks']=$marks;
					...
					do_action('wplms_quiz_results_per_question_html',$quiz_id,$question,$user_id);
	...
	echo '<div id="total_marks">'.__('Total Marks','wplms').' '.($quiz_status?'<strong><span>'.$sum.'</span>'.(empty($total_sum)?'':' / '.$total_sum).'</strong>':'<strong><i class="icon-alarm"></i></strong>').' </div>';
...
function bp_course_quiz_retake_form($quiz_id,$user_id,$course=NULL){
	...
	if(isset($retakes) && $retakes){
		...
		if( ($retakes - $retake_count) > 0){
			...
			echo '<input type="submit" name="initiate_retake" value="'.__('RETAKE QUIZ','wplms').'" />';
			echo '<p id="retakes'.$retakes.'">'.__('Number of retakes ','wplms').' : <strong>'.$retake_count.__(' out of ','wplms').$retakes.'</strong></p>';
			wp_nonce_field('retake'.$user_id,'security');
			echo '</form>';
		}

		echo '<h4 id="prev_results"><a href="#">'.__('Previous Results for Quiz ','wplms').'</a></h4>';
		...
		if(count($quiz_results) > 0){
			...
			echo '<div id="message"><p>'.__('Results not Available','wplms').'</p></div>';
		}
	} // END Retakes
}