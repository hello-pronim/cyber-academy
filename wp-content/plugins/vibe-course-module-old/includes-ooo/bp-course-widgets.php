...
function bp_course_register_widgets() {
    ...
    register_widget('BP_Course_Certficates_Widget');
...
class BP_Course_Widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		<select id="<?php echo $this->get_field_id( 'orderby' ); ?>" name="<?php echo $this->get_field_name( 'orderby' ); ?>">
			...
			<option value="vibe_start_date" <?php selected('vibe_start_date',$orderby ); ?>><?php _e('Upcoming Courses (Start Date)','vibe'); ?></option>
			<option value="vibe_featured" <?php selected('vibe_featured',$orderby ); ?>><?php _e('Featured','vibe'); ?></option>
		</select>
		</p>
...
class BP_Course_Filter_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
			...
			if(isset($categories) && is_array($categories)){
				...
				foreach($categories as $category){
					if(!in_array($category->slug,$exclude_array)){
						...
						<div class="checkbox">
							<input id="'.$category->slug.'" type="checkbox" data-filter-type="course-cat" class="course_filter_item bp-course-category-filter" name="bp-course-category-filter" value="'.$category->slug.'" /> 
						...
						if(isset($sub_categories) && is_array($sub_categories) && count($sub_categories)){
							...
							foreach($sub_categories as $sub_category){
								echo '<li>
								<div class="checkbox">
									<input id="'.$sub_category->slug.'" type="checkbox" class="bp-course-category-filter" name="course_filter_item bp-course-category-filter" value="'.$sub_category->slug.'" /> 
			...
			if(isset($categories) && is_array($categories)){
				...
				foreach($categories as $category){
					if(!in_array($category->slug,$exclude_array)){
						echo '<li>
						<div class="checkbox">
							<input id="'.$category->slug.'" type="checkbox" data-filter-type="location"  class="course_filter_item bp-course-location-filter" name="bp-course-category-level" value="'.$category->slug.'" /> 
							<label for="'.$category->slug.'">'.$category->name.(!empty($show_post_count)?' ('.$category->count.')':'').'</label>
						</div></li>';
					}
				}
			...
			if(isset($categories) && is_array($categories)){
				...
				foreach($categories as $category){
					if(!in_array($category->slug,$exclude_array)){
						echo '<li>
						<div class="checkbox">
							<input id="'.$category->slug.'" type="checkbox" data-filter-type="level" class="course_filter_item bp-course-level-filter" name="bp-course-category-level" value="'.$category->slug.'" /> 
							<label for="'.$category->slug.'">'.$category->name.(!empty($show_post_count)?' ('.$category->count.')':'').'</label>
						</div></li>';
					}
				}
		...
		do_action('bp_course_filter_widget' ,$args, $instance);
...
class BP_Course_Certficates_Widget extends WP_Widget {

	function __construct() {
	  $widget_ops = array( 'classname' => 'BuddyPress Course Certificates', 'description' => 'Displays Recent Certificates.' );
	  $control_ops = array( 'width' => 250, 'height' => 350,'id_base' => 'bp_course_certificates_widget');
	  parent::__construct( 'bp_course_certificates_widget',  __('BuddyPress Course Certificates Widget','vibe'), $widget_ops, $control_ops);
	  }
	...
	function widget( $args, $instance ) {
		...
		echo $before_title .
		     ...
		     if($course_ids == 1 && (is_singular('course')|| bp_current_component() == 'course')){
		     	global $post;
				$course_ids = $post->ID;
		     }

		     if(is_numeric(($course_ids)) && $course_ids > 1){
		    ...
		    if(is_array($course_ids)){
		    	 ...
		    	 $certificates = $wpdb->get_results("
			    	...
			    	AND post_type = 'attachment' AND post_name LIKE 'certificate_%'");
	    	}else{
	    		$certificates = $wpdb->get_results("
		    	SELECT ID FROM {$wpdb->posts} 
		    	WHERE post_type = 'attachment' AND post_name LIKE 'certificate_%'");
	    	}
		   ...
		   $ids = array();
		   	if(empty($certificates)){
		   	 _e('No certificates awarded for course yet !','vibe');
		   	}else{
		   		...
		   		if($style == 'carousel'){
		   			?>
		   			<div class="widget_carousel flexslider  no-ajax">
		   				<ul class="slides">
		   					<?php
		   						foreach($ids as $id){
		   							echo '<li><img src="'.wp_get_attachment_url($id).'" alt="'.get_the_title($id).'" /></li>';
		   						}
		   					?>
		   				</ul>
		   			</div>
	...
	function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$instance['title'] = strip_tags( $new_instance['title'] );
		$instance['course_ids'] = strip_tags( $new_instance['course_ids'] );
		$instance['max_items'] = strip_tags( $new_instance['max_items'] );
		$instance['style'] = strip_tags( $new_instance['style'] );
	...
	function form( $instance ) {
		$defaults = array( 'title'=> 'Recent Certificates',
			'course_ids' => '1','max_items' => 5,'style'=>'grid' );

		$instance = wp_parse_args( (array) $instance, $defaults );
		extract( $instance, EXTR_SKIP );
		$title = esc_attr($instance['title']);
		?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Widget Title', 'vibe' ); ?> <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" style="width: 30%" /></label></p>
		<p><label for="<?php echo $this->get_field_id( 'course_ids' ); ?>"><?php _e( 'Course IDs ( comma saperated), Set 1 for current course', 'vibe' ); ?> <input class="widefat" id="<?php echo $this->get_field_id( 'course_ids' ); ?>" name="<?php echo $this->get_field_name( 'course_ids' ); ?>" type="text" value="<?php echo esc_attr( $course_ids ); ?>" style="width: 30%" /></label></p>
		<p><label for="bp-instructor-widget-max"><?php _e( 'Max. Number of Certificates to show', 'vibe' ); ?> <input class="widefat" id="<?php echo $this->get_field_id( 'max_items' ); ?>" name="<?php echo $this->get_field_name( 'max_items' ); ?>" type="text" value="<?php echo esc_attr( $max_items ); ?>" style="width: 30%" /></label></p>
		<p><label for="bp-course-related-courses-widget-style"><?php _e( 'Style', 'vibe' ); ?> </label>
		<select id="<?php echo $this->get_field_id( 'style' ); ?>" name="<?php echo $this->get_field_name( 'style' ); ?>"><option value="grid" <?php selected('grid',$style); ?>><?php _e( 'Grid', 'vibe' ); ?></option><option value="carousel" <?php selected('carousel',$style); ?>><?php _e( 'Carousel', 'vibe' ); ?></option>
		</select>
		</p>
	<?php
	}
