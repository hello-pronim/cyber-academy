 ...
 */
  function bp_wplms_get_quiz_data($user_id,$item_id,$item=null,$course=null){
    ...
    if($quiz_access_flag){
      ...
      $return['quiz_passing_score'] = $quiz_passing_score;
...
function the_course_button($id=NULL){
  ...
  if(isset($id) && $id)
   ...
   wp_enqueue_script('courseButton',plugins_url('../../../assets/js/course_button.js',__FILE__),array('wp-element','wp-data'),rand(0,999),true);
   ...
   wp_localize_script('courseButton','coursebutton',array(
  ...
  if(!is_user_logged_in() && vibe_validate($free_course)){
    echo apply_filters('wplms_course_non_loggedin_user','<span class="the_course_button" data-id="'.$course_id.'"><a href="'.(is_wplms_4_0()?'#':get_permalink($course_id).'?error=login').'" class="course_button  is-loading button full '.((function_exists('vibe_get_option') && vibe_get_option('enable_ajax_registration_login'))?'auto_trigger':'').'">'.apply_filters('wplms_take_this_course_button_label',__('TAKE THIS COURSE','wplms'),$course_id).'</a></span>',$course_id); 
   ...
   if(!empty($coursetaken) && is_user_logged_in()){   // COURSE IS TAKEN & USER IS LOGGED IN
         ...
         if($coursetaken > time()){  // COURSE ACTIVE
            ...
            echo apply_filters('wplms_expired_course_button','<a href="'.$pid.'" class="'.((isset($id) && $id )?'course_button':'course_button full  is-loading').' button">'.__('Course Expired','wplms').'&nbsp;<span>'.__('Click to renew','wplms').'</span></a>',$course_id);   
      }
      ...
      if(isset($pid) && $pid){
        ...
        if( vibe_validate($partial_free_course) && is_user_logged_in() ){
          echo apply_filters('wplms_take_course_button_html','<a href="'.get_permalink($course_id).'?subscribe" class="course_button full button  is-loading">'.apply_filters('wplms_take_this_course_button_label',__('SUBSCRIBE FOR FREE','wplms'),$course_id).apply_filters('wplms_course_button_extra',$extra,$course_id).'</a>',$course_id);
        }else{
          echo apply_filters('wplms_take_course_button_html','<span class="the_course_button" data-id="'.$course_id.'"><a href="'.$pid.'" class="course_button full button  is-loading">'.apply_filters('wplms_take_this_course_button_label',__('TAKE THIS COURSE','wplms'),$course_id).apply_filters('wplms_course_button_extra',$extra,$course_id).'</a></span>',$course_id);
        }
      }else{
...
if(!function_exists('the_question')){
  function the_question($id=null,$quiz_id=null){
    ...
    if(isset($hint) && strlen($hint)>5){
      ...
      echo '<div class="hint"><i><span class="left">'.__('HINT','wplms').' : </span>'.do_shortcode(apply_filters('the_content',$hint)).'</i></div>';
    }
    echo '</div>';
