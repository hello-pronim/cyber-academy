...
function bp_course_activity_actions(){
    return apply_filters('bp_course_Register_actions',array(
        'remove_from_course' => __( 'Removed a student from Course', 'wplms' ),
        'submit_course' => __( 'Student submitted a Course', 'wplms' ),
        'start_course' => __( 'Student started a Course', 'wplms' ),
        'submit_quiz' => __( 'Student submitted a Quiz', 'wplms' ),
        'start_quiz' => __( 'Student started a Quiz', 'wplms' ),
        'unit_complete' => __( 'Student completed a unit', 'wplms' ),
        'reset_course' => __( 'Course reset for Student', 'wplms' ),
        'bulk_action' => __( 'Bulk action by instructor', 'wplms' ),
        'course_evaluated' => __( 'Course Evaluated for student', 'wplms' ),
        'student_badge'=> __( 'Student got a Badge', 'wplms' ),
        'student_certificate' => __( 'Student got a certificate', 'wplms' ),
        'quiz_evaluated' => __( 'Quiz Evaluated for student', 'wplms' ),
        'subscribe_course' => __( 'Student subscribed for course', 'wplms' ),
        'course_application'=> __( 'Student applied for course', 'wplms' ),
        'course_application_approve'=> __( 'Student application approved for course', 'wplms' ),
        'course_application_reject'=> __( 'Student applicaiton rejected for course', 'wplms' ),
...
class bp_course_activity{
	...
	function wplms_dashboard_course_announcement($course_id,$student_type,$email,$announcement){
		bp_course_record_activity(array(
		      'action' => sprintf(__('Announcement for Course %s ','wplms'),get_the_title($course_id)),
	...
	function wplms_course_news($news_id,$post){
		...
		bp_course_record_activity(array(
		      'action' => sprintf(__('News published for Course %s ','wplms'),get_the_title($course_id)),
	...
	function course_subscribed($course_id,$user_id,$group_id = null,$args=null){
		...
		$activity_id = bp_course_record_activity(array(
		      'action' => sprintf(__('Student subscribed for course %s','wplms'),get_the_title($course_id)),
		      'content' => sprintf(__('Student %s subscribed for course %s','wplms'),bp_core_get_userlink( $user_id ),get_the_title($course_id)),
	...
	function start_course($course_id,$user_id = NULL){
		...
		$activity_id=bp_course_record_activity(array(
            'action' => __('Student started course ','wplms').get_the_title($course_id),
            'content' => sprintf(_x('Student %s started the course %s','Recoreded activity message','wplms'),bp_core_get_userlink( $user_id ),get_the_title($course_id)),
            'type' => 'start_course',
            'item_id' => $course_id,
            'primary_link'=>get_permalink($course_id),
            'secondary_item_id'=>$user_id
	...
	function submit_course($course_id,$user_id){
		...
		$message = sprintf(__('Student %s finished and submitted the course %s','wplms'),bp_core_get_userlink($user_id), get_the_title($course_id));
		bp_course_record_activity(array(
	          'action' => __('Student Submitted the course ','wplms'),
	          'content' => $message,
	          'type' => 'submit_course',
	          'item_id' => $course_id,
	          'primary_link'=>get_permalink($course_id),
	...
	function evaluate_course($course_id,$marks,$user_id){
		...
		$activity_id=bp_course_record_activity(array(
	      'action' => __('Instructor evaluated Course for student','wplms'),
	      'content' => sprintf(__('Student %s got %s in course %s','wplms'),bp_core_get_userlink( $user_id ),apply_filters('wplms_course_marks',$marks.'/100',$course_id),get_the_title($course_id)),
	      'primary_link' => get_permalink($course_id),
	...
	function course_reset($course_id,$user_id){
		bp_course_record_activity(array(
	      'action' => __('Course reset for student ','wplms'),
	      'content' => sprintf(__('Course %s is reset for student %s','wplms'),get_the_title($course_id),bp_core_get_userlink($user_id)),
	      'type' => 'reset_course',
	      'primary_link' => get_permalink($course_id),
	      'item_id' => $course_id,
	      'secondary_item_id' => $user_id
	...
	function course_retake($course_id,$user_id){
      	bp_course_record_activity(array(
	      'action' => __('Student retake Course ','wplms'),
	      'content' => sprintf(__('Course %s is retaken by student %s','wplms'),get_the_title($course_id),bp_core_get_userlink($user_id)),
	      'type' => 'retake_course',
	      'primary_link' => get_permalink($course_id),
	      'item_id' => $course_id,
	      'secondary_item_id' => $user_id
	...
	function badge_earned($course_id,$badges,$user_id,$badge_filter){
		bp_course_record_activity(array(
          'action' => __('Student got a Badge in the course ','wplms'),
          'content' => sprintf(__('Student %s got a badge in the course %s','wplms'),bp_core_get_userlink($user_id),get_the_title($course_id)),
          'type' => 'student_badge',
          'item_id' => $course_id,
          'primary_link'=>get_permalink($course_id),
        )); 
	...
	function certificate_earned($course_id,$pass,$user_id,$passing_filter){
		bp_course_record_activity(array(
            'action' => __('Student got a Certificate in course','wplms'),
            'content' => sprintf(__('Student %s got a certificate in the course %s','wplms'),bp_core_get_userlink($user_id),get_the_title($course_id)),
	...
	function course_review($course_id,$rating,$title){
		$user_id = get_current_user_id();
		bp_course_record_activity(array(
	      'action' => sprintf(__('Student reviewed Course %s','wplms'),get_the_title($course_id)),
	      'content' => sprintf(__('Student %s reviewed the Course %s','wplms'),bp_core_get_userlink($user_id),get_the_title($course_id)),
	      'type' => 'review_course',
	      'primary_link' => get_permalink($course_id),
	      'item_id' => $course_id,
	      'secondary_item_id' => $user_id
	...
	function course_unsubscribe($course_id,$user_id,$group_id = null){
		...
		if(empty($group_id))
			$group_id = '';
		bp_course_record_activity(array(
	      'action' => __('Student unsubsribed from course ','wplms'),
	      'content' => sprintf(__('Student %s is removed from the course %s','wplms'),bp_core_get_userlink($user_id),get_the_title($course_id)),
	...
	function course_codes($code,$course_id,$user_id){
		$activity_id = bp_course_record_activity(array(
          'action' => __('Course code applied','wplms'),
          'content' =>sprintf(__('Student %s applied Course code for course %s','wplms'),bp_core_get_userlink($user_id),get_the_title($course_id)),
          'type' => 'course_code',
          'item_id' => $course_id,
          'primary_link'=>get_permalink($course_id),
          'secondary_item_id'=>$user_id
	...
	function renew_course($course_id,$user_id){
		bp_course_record_activity(array(
            'action' => __('Student renewed a course','wplms'),
            'content' => sprintf(__('Student %s renewed the course %s','wplms'),bp_core_get_userlink($user_id),get_the_title($course_id)),
            'type' => 'renew_course',
            'item_id' => $course_id,
            'primary_link'=>get_permalink($course_id),
            'user_id'=>$user_id
	...
	function start_unit($unit_id,$course_id,$user_id){
		bp_course_record_activity(array(
          'action' => __('Student started a unit','wplms'),
          'content' => sprintf(__('Student started the unit %s in course %s','wplms'),get_the_title($unit_id),get_the_title($course_id)),
          'type' => 'start_unit',
	...
	function unit_complete($unit_id,$course_progress,$course_id,$user_id){
		$activity_id=bp_course_record_activity(array(
	      ...
	      'content' => sprintf(__('Student %s finished the unit %s in course %s','wplms'),bp_core_get_userlink($user_id),get_the_title($unit_id),get_the_title($course_id)),
	      'type' => 'unit_complete',
	      'primary_link' => get_permalink($unit_id),
	      'item_id' => $course_id,
	      'secondary_item_id' => $unit_id
	...
	function unit_instructor_complete($unit_id,$user_id,$course_id){
		...
		bp_course_record_activity(array(
	      'action' => __('Instructor completed a unit for Student ','wplms'),
	      'content' => sprintf(__('Instructor %s completed unit %s for Student %s in course %s','wplms'),bp_core_get_user_displayname($instructor_id),get_the_title($unit_id),bp_core_get_user_displayname($user_id),get_the_title($course_id)),
	...
	function unit_instructor_uncomplete($unit_id,$user_id,$course_id){
		$instructor_id = get_current_user_id();
		bp_course_record_activity(array(
	      'action' => __('Instructor uncompleted a unit for Student ','wplms'),
	      'content' => sprintf(__('Instructor %s uncompleted unit %s for Student %s in course %s','wplms'),bp_core_get_user_displayname($instructor_id),get_the_title($unit_id),bp_core_get_user_displayname($user_id),get_the_title($course_id)),
	      'type' => 'unit_instructor_complete',
	      'primary_link' => get_permalink($unit_id),
	      'item_id' => $course_id,
	      'secondary_item_id' => $unit_id,
	...
	function unit_comment($unit_id,$user_id,$comment_id,$course_id = null){
		if(empty($course_id)){
			$course_id = bp_course_get_unit_course_id($unit_id);	
		}
		bp_course_record_activity(array(
          'action' => __('Student posted comment on unit ','wplms'),
          'content' => sprintf(__('Student %s posted comment on unit %s','wplms'),bp_core_get_user_displayname($user_id),get_the_title($unit_id)),
          'type' => 'unit_comment',
          'item_id' => $course_id,
          'primary_link'=>get_permalink($course_id),
	...
	function start_quiz($quiz_id,$user_id){
		...
		bp_course_record_activity(array(
          'action' => __('Student started a quiz','wplms'),
          'content' => sprintf(__('Student %s started the quiz %s','wplms'),bp_core_get_userlink($user_id),get_the_title($quiz_id)),
          'type' => 'start_quiz',
	...
	function submit_quiz($quiz_id,$user_id){
		$id = get_post_meta($quiz_id,'vibe_quiz_course',true);
		bp_course_record_activity(array(
	      'action' => __('Student submitted the Quiz','wplms'),
	      'content' => sprintf(__('Quiz %s was submitted by student %s','wplms'),get_the_title($quiz_id),bp_core_get_userlink( $user_id )),
	      'type' => 'submit_quiz',
	      'primary_link' => get_permalink($quiz_id),
	      'item_id' => empty($id)?$quiz_id:$id,
          'secondary_item_id' => $quiz_id,
	...
	function evaluate_quiz($quiz_id,$marks,$user_id,$max){
		$id = get_post_meta($quiz_id,'vibe_quiz_course',true);
	    $activity_id=bp_course_record_activity(array(
	      'action' => __('Instructor evaluated Quiz for student ','wplms'),
	      'type' => 'quiz_evaluated',
	      'content' => sprintf(__('Student %s got %s out of %s in Quiz %s','wplms'),bp_core_get_userlink( $user_id ),$marks,$max,get_the_title($quiz_id)),
	...
	function quiz_retake($quiz_id,$user_id){
		$id = get_post_meta($quiz_id,'vibe_quiz_course',true);
		bp_course_record_activity(array(
	        'action' => __('Quiz retake by Student','wplms'),
	        'content' => sprintf(__('Student %s  initiated retake for quiz %s','wplms'),bp_core_get_userlink( $user_id ),get_the_title($quiz_id)),
	        'type' => 'retake_quiz',
	        'primary_link' => get_permalink($quiz_id),
	        'item_id' => empty($id)?$quiz_id:$id,
          	'secondary_item_id' => $quiz_id,
	...
	function quiz_reset($quiz_id,$user_id){
		$id = get_post_meta($quiz_id,'vibe_quiz_course',true);
		bp_course_record_activity(array(
	      'action' => __('Instructor Reseted the Quiz for User','wplms'),
	      'content' => sprintf(__('Quiz %s  was reset by the Instructor for user %s','wplms'),get_the_title($quiz_id),bp_core_get_userlink( $user_id )),
	      'type' => 'reset_quiz',
	      'primary_link' => get_permalink($quiz_id),
	      'item_id' => empty($id)?$quiz_id:$id,
          'secondary_item_id' => $quiz_id,
	...
	function bulk_action($action,$course_id,$members){
		switch($action){
			case 'added_students':
				$activity_id =bp_course_record_activity(array(
			      'action' => __('Instructor added students in course  ','wplms'),
			      'content' => sprintf(__('Instructor added %s students in course ','wplms'),count($members)),
			      'type' => 'bulk_action',
			      'item_id' => $course_id,
			      ));
			break;
			case 'change_course_status':
				$activity_id =bp_course_record_activity(array(
		        'action' => __('Instructor changed course status  ','wplms'),
		        'content' => sprintf(__('Instructor changed Course Status for %s students in course ','wplms'),count($members)),
		        'type' => 'bulk_action',
		        'item_id' => $course_id,
		        ));
			break;
			case 'bulk_message':
				$activity_id =bp_course_record_activity(array(
			      'action' => __('Instructor sent Bulk message to students','wplms'),
			      'content' => sprintf(__('Bulk Message sent to %s students ','wplms'),count($members)),
			      'type' => 'bulk_action',
			      'item_id' => $course_id,
			      ));
			break;
			case 'extend_course_subscription':
				$activity_id =bp_course_record_activity(array(
			      'action' => __('Instructor extended Course subscription','wplms'),
			      'content' => sprintf(__('Course subscription extended for %s students ','wplms'),count($members)),
			      'type' => 'bulk_action',
			      'item_id' => $course_id,
			      ));
			break;
			...
			case 'remove_certificate':
				$activity_id = bp_course_record_activity(array(
			        'action' => __('Instructor assigned/removed Certificate/Badges','wplms'),
			        'content' => sprintf(__('Instructor added/removed Badges/Certificates from %s students in course','wplms'),count($members)),
			        'type' => 'bulk_action',
			        'item_id' => $course_id,
			        ));
			break;
	...
	function start_assignment($assignment_id,$user_id){
		...
		$activity_id=bp_course_record_activity(array(
          'action' => sprintf(__('Student started assignment %s','wplms'),get_the_title($assignment_id)),
          'content' => sprintf(__('Student %s started the assignment %s','wplms'),bp_core_get_userlink($user_id),get_the_title($assignment_id)),
	...
	function submit_assignment($assignment_id,$user_id){
		...
		$instructor_id=get_post_field('post_author', $assignment_id);
		$activity_id=bp_course_record_activity(array(
          'action' => __('Student submitted assignment ','wplms'),
          'content' => sprintf(__('Student %s submitted the assignment %s','wplms'),bp_core_get_userlink($user_id),get_the_title($assignment_id)),
	...
	function evaluate_assignment($assignment_id,$marks,$user_id,$max,$message_id){
		$course_id = get_post_meta($assignment_id,'vibe_assignment_course',true);
		$activity_id=bp_course_record_activity(array(
	      'action' => sprintf(__('Results available for assignment %s','wplms'),get_the_title($assignment_id)),
	      'content' => sprintf(__('Student %s got marks %s out of %s in assignment %s ','wplms'),bp_core_get_userlink( $user_id ),$marks,$max,get_the_title($assignment_id)),
	...
	function reset_assignment($assignment_id,$user_id){
		$course_id = get_post_meta($assignment_id,'vibe_assignment_course',true);
		bp_course_record_activity(array(
	      'action' => __('Instructor Reseted the Assignment for User','wplms'),
	      'content' => sprintf(__('Assignment %s was reset by the Instructor for user %s','wplms'),get_the_title($assignment_id),bp_core_get_userlink( $user_id )),
	...
	function record_cpt_activity_action( $activity_action,  $post, $post_permalink ) {
		...
		if( $post->post_type != 'post' ) {
			if ( is_multisite() )
			$activity_action  = sprintf( __( '%1$s wrote a new %2$s, %3$s, on the site %4$s', 'wplms' ), bp_core_get_userlink( (int) $post->post_author ), $post->post_type, '<a href="' . $post_permalink . '">' . $post->post_title . '</a>', get_blog_option( $blog_id, 'blogname' ));
			else
			$activity_action  = sprintf( __( '%1$s wrote a new %2$s, %3$s', 'wplms' ), bp_core_get_userlink( (int) $post->post_author ),$post->post_type, '<a href="' . $post_permalink . '">' . $post->post_title . '</a>' );
		} 
	...
	function record_cpt_comment_activity_action( $activity_action,  $recorded_comment, $comment_link ) {
		...
		if( $recorded_comment->post->post_type == 'course' ) {
			if ( is_multisite() )
				$activity_action = sprintf( __( '%1$s reviewed the %2$s, %3$s, on the site %4$s', 'wplms' ), bp_core_get_userlink( $user_id ), $recorded_comment->post->post_type, '<a href="' . $post_permalink . '">' . apply_filters( 'the_title', $recorded_comment->post->post_title ) . '</a>', '<a href="' . get_blog_option( $blog_id, 'home' ) . '">' . get_blog_option( $blog_id, 'blogname' ) . '</a>' );
			else
				$activity_action = sprintf( __( '%1$s reviewed the %2$s, %3$s', 'wplms' ), bp_core_get_userlink( $user_id ),$recorded_comment->post->post_type, '<a href="' . $post_permalink . '">' . apply_filters( 'the_title', $recorded_comment->post->post_title ) . '</a>' );
		} 
		if( $recorded_comment->post->post_type == 'question' ) {
			if ( is_multisite() )
				$activity_action = sprintf( __( '%1$s answered the %2$s, %3$s, on the site %4$s', 'wplms' ), bp_core_get_userlink( $user_id ), $recorded_comment->post->post_type, '<a href="' . $post_permalink . '">' . apply_filters( 'the_title', $recorded_comment->post->post_title ) . '</a>', '<a href="' . get_blog_option( $blog_id, 'home' ) . '">' . get_blog_option( $blog_id, 'blogname' ) . '</a>' );
			else
				$activity_action = sprintf( __( '%1$s answered the %2$s, %3$s', 'wplms' ), bp_core_get_userlink( $user_id ),$recorded_comment->post->post_type, '<a href="' . $post_permalink . '">' . apply_filters( 'the_title', $recorded_comment->post->post_title ) . '</a>' );
		} 
		return $activity_action;
	}
	...
	function user_course_application($course_id,$user_id){
		bp_course_record_activity(array(
		      'action' => sprintf(__('User applied for Course %s ','wplms'),get_the_title($course_id)),
		      'content' => sprintf(__('User %s applied for Course %s ','wplms'),bp_core_get_userlink( $user_id ),get_the_title($course_id)),
	...
	function manage_user_application($action,$user_id,$course_id){
		bp_course_record_activity(array(
		      'action' => sprintf(__('User application %s for Course %s ','wplms'),$action,get_the_title($course_id)),
		      'content' => sprintf(__('User %s application %s for Course %s ','wplms'),bp_core_get_userlink( $user_id ),$action,get_the_title($course_id)),
	...
	function course_go_live($course_id,$the_post){
		if($the_post['post_status'] == 'publish'){
			bp_course_record_activity(array(
			      'action' => sprintf(__('Instructor Published the course Course %s ','wplms'),$the_post['post_title']),
			      'content' => sprintf(__('Instructor %s published the Course %s ','wplms'),bp_core_get_userlink( $the_post['post_author'] ),$the_post['post_title']),
			...
			bp_course_record_activity(array(
			      'action' => sprintf(__('Instructor submitted the Course %s for approval','wplms'),$the_post['post_title']),
			      'content' => sprintf(__('Instructor %s submitted the Course %s for approval','wplms'),bp_core_get_userlink( $the_post['post_author'] ),$the_post['post_title']),
