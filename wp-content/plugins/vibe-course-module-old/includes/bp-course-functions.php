 ...
 */
  function bp_wplms_get_quiz_data($user_id,$item_id,$item=null,$course=null){
    //Get all questions.
    if(!empty($user_id)){
    ...
    if($quiz_access_flag){
      ...
      if(!empty($all_questions)){
        ...
        $quiz_duration_parameter = apply_filters('vibe_quiz_duration_parameter',86400,$item_id);
        ...
        foreach($all_questions['ques'] as $k=>$question_id){
          ...
          if($status  >= 3){
            ...
            ob_start();
            bp_course_quiz_retake_form($item_id,$user_id);
            $retake_html = ob_get_clean();
            $return['retake_html'] =  $retake_html;
          }else{
            ...
            if($question['type'] && $question['type'] == 'truefalse'){
              $question['options'] = array(_x('False','','vibe'),_x('True','','vibe'));
            }
      ...
      $return['partial_marking'] = $quiz_partial_marks;
      $return['negative_marking'] = $quiz_negative_marking;
      $return['negative_marks'] = $negative_marks;
      $return['question_number'] = $vibe_question_number_react;
      $return['show_advance_stats'] = $show_question_advanced_stats_in_quiz;
      ...
      $return['content'] = _x('Quiz already in progress, contact site, please retry after sometime.','quiz lock flag check for App and Site','vibe');
      $return['meta'] = array(
        'access' => 0
      );
    }
...
function the_course_button($id=NULL){
  ...
  if(!is_user_logged_in() && vibe_validate($free_course)){
    echo apply_filters('wplms_course_non_loggedin_user','<a href="'.get_permalink($course_id).'?error=login" class="course_button button full '.((function_exists('vibe_get_option') && vibe_get_option('enable_ajax_registration_login'))?'auto_trigger':'').'">'.apply_filters('wplms_take_this_course_button_label',__('TAKE THIS COURSE','vibe'),$course_id).apply_filters('wplms_course_button_extra',$extra,$course_id).'</a>',$course_id); 
    return;
  }
   ...
   if(!empty($coursetaken) && is_user_logged_in()){   // COURSE IS TAKEN & USER IS LOGGED IN
      ...
      if(isset($pid) && $pid){
        ...
        echo apply_filters('wplms_private_course_button_html','<a href="'.apply_filters('wplms_private_course_button','#',$course_id).'" class="'.((isset($id) && $id )?'':'course_button full ').'button">'. apply_filters('wplms_private_course_button_label',__('PRIVATE COURSE','vibe'),$course_id).apply_filters('wplms_course_button_extra',$extra,$course_id).'</a>',$course_id); 
      }
   }
}
}
...
if(!function_exists('the_unit')){
  function the_unit($id=NULL){
    ...
    if(class_exists('WPLMS_tips')){
      ...
      $tips = WPLMS_tips::init();
      if(is_user_logged_in() && isset($tips) && isset($tips->settings) && !empty($tips->settings['react_quizzes']) && !has_shortcode(get_post_field('post_content',$id), 'wplms_h5p' )){
          $react_quizzes = 1;
      }
    }
                ...
                if(isset($question) && $question){
                    ...
                    if($type == 'survey'){
                        ...
                        if($check){
                          ...
                          $total_marks = $total_marks+$marks;
                          update_question_correct_percentage($question);
                          ...
                          $total_marks = $total_marks+$marks;
                          update_question_incorrect_percentage($question);
                        }
...
function bp_course_instructor_controls(){
  ...
  echo '<li><a href="'.untrailingslashit(get_permalink($course_id)).$extend_submissions.'" class="action_icon tip" title="'.__('Evaluate course submissions','vibe').'"><i class="icon-task"></i><span>'.$num.'</span></a></li>';  
  ...
  if(isset($course_quizes) && !empty($course_quizes) && is_array($course_quizes) && count($course_quizes)){
    ...
    echo '<li><a href="'.untrailingslashit(get_permalink($course_id)).$extend_submissions.'" class="action_icon tip"  title="'.__('Evaluate Quiz submissions','vibe').'"><i class="icon-check-clipboard-1"></i><span>'.$num.'</span></a></li>';

  } 

  $n=get_post_meta($course_id,'vibe_students',true);
  if(isset($n) && $n !=''){$num=$n;}else{$num=0;}
  echo '<li><a href="'.untrailingslashit(get_permalink($course_id)).$admin_slug.'" class="action_icon tip"  title="'.__('Manage Students','vibe').'"><i class="icon-users"></i><span>'.$num.'</span></a></li>';
  echo '<li><a href="'.untrailingslashit(get_permalink($course_id)).$extend_stats.'" class="action_icon tip"  title="'.__('See Stats','vibe').'"><i class="icon-analytics-chart-graph"></i></a></li>';
  echo '<li><a href="'.untrailingslashit(get_permalink($course_id)).$extend_activity.'" class="action_icon tip"  title="'.__('See all Activity','vibe').'"><i class="icon-atom"></i></a></li>';
  echo '</ul>';
}
...
function bp_course_get_course_submission_count($course_id){
  global $wpdb;
  $count = $wpdb->get_var(apply_filters('wplms_usermeta_direct_query',"SELECT count(*) FROM {$wpdb->usermeta} WHERE meta_key = 'course_status$course_id' AND meta_value LIKE '3'"));
...
function bp_course_get_quiz_submission_count($course_id){
  ...
  if(!empty($quizes)){
    ...
    $datas = $wpdb->get_results(apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT u.meta_key as quiz_id, u.user_id as user_id FROM {$wpdb->postmeta} as p LEFT JOIN {$wpdb->usermeta} as u ON p.meta_key = u.user_id WHERE p.meta_value LIKE '0' AND u.meta_key = p.post_id AND u.meta_value < %d AND p.post_id IN ($quiz_ids)",time())));
    $count = 0;
    if(!empty($datas)){
      foreach($datas as $data){
        $quiz_status = bp_course_get_user_quiz_status($data->user_id,$data->quiz_id);
          if($quiz_status != 4 ) {
            $count++;
          }
      }
    ...
    function bp_course_record_instructor_commission($instructor_id,$commission,$course_id,$meta){
        ...
        if(!empty($instructor_id) && !empty($commission_html) && !empty($course_id)){
          if($commission > 0){
            ...
            $activity_id = bp_course_record_activity(apply_filters('bp_course_record_instructor_commission_activity',array(
                'user_id' => $instructor_id,
                'action' => _x('Commission reversed.','Instructor earned commission activity','vibe'),
                'content' => sprintf(_x('%s commission reversed for course %s  for order ID %d as order cancelled/refunded','Instructor earned commission activity','vibe'),$commission_html,get_the_title($course_id),$meta['order_id']),
                'component' => 'course',
                'type' => 'course_commission',
                'item_id' => $course_id,
                'secondary_item_id' => $secondary_item_id,
...
function wplms_get_course_schema_details($course_id){
    ...
    if(!empty($certificate_set)){
        $course_data["educationalCredentialAwarded"] = array(
            ...
            "name" =>sprintf(_x('Certificate for %s','','vibe'),$title),
            "educationalLevel" => array(
                "@type"=> "DefinedTerm",
                "name"=> sprintf(_x('Certification in %s','','vibe'),$title),
        ...
        foreach ($course_comments as  $course_comment) {
            ...
            if(!empty($comment_rating)){
                $course_data["review"][]= array (
                   ...
                   "author"=>array(
                        "@type"=> "Person",
                        "name"=>((!empty($course_comment->user_id)?bp_core_get_user_displayname($course_comment->user_id):((!empty($course_comment->comment_author))?$course_comment->comment_author:_x('Anonymous','','vibe'))))
                   )
                ); 
            }
