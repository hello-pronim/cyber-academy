      ...
      if(bp_course_is_member($_POST['course_id'],$_POST['user_id'])){
        ...
        $time = apply_filters('wplms_force_complete_unit',time(),$_POST['id'],$_POST['course_id'],$_POST['user_id']);
          ...
          if($post_type == 'unit'){
            ...
            bp_course_update_user_quiz_status($_POST['user_id'],$_POST['id'],4);
        ...
        if($being > 3){
          ...
          echo '</h6>';
              ...
              if($type == 'unit'){
                ...
                if(is_numeric($marks)){
                    $complete++;
  							    $curriculum .= '<li class="check_user_quiz_results" data-quiz="'.$c.'" data-user="'.$user_id.'"><span class="done" data-id="'.$c.'"></span> '.get_the_title($c).' <strong>'.(($marks)?_x('Marks Obtained : ',' marks obtained in quiz result','vibe').$marks:__('Under Evaluation','vibe')).'</strong></li>';
                    }else{
                        $curriculum .= '<li><span data-id="'.$c.'"></span> '.get_the_title($c).'</li>';    
                    }
                }
  ...
  function wplms_check_user_quiz_results($quiz_id = NULL,$user_id = NULL){
    ...
    function evaluate_quiz(){
          ...
          foreach($results as $question_key=>$question){
            ...
            echo '<li>
              ...
              if(isset($correct_answer) && $correct_answer !='' && isset($marked_answer) && $marked_answer != ''){
                  echo '<strong>';_e('Correct Answer :','vibe');echo '<span>'.$correct_answer.'</span></strong>';
              }
              ...
              if(!empty($marks)){
                if(!isset($results[$marked_answer_id]))
                    ...
                    <a href="#" class="give_marks button" data-from-activity="'.$activity_id.'" data-ans-id="'.$marked_answer_id.'" data-quiz="'.$quiz_id.'" data-user="'.$user_id.'">'.__('Update Marks','vibe').'</a>';
                ...
                if(!isset($results[$marked_answer_id])){
                    ...
                    <a href="#" class="give_marks button" data-from-activity="'.$activity_id.'" data-ans-id="'.$marked_answer_id.'" data-quiz="'.$quiz_id.'" data-user="'.$user_id.'">'.__('Give Marks','vibe').'</a>';
                }
              }
      ...
      if(!empty($questions) || !empty($results)):
        ...
        echo '<a href="#" id="mark_complete" class="button full" data-from-activity="'.$activity_id.'" data-quiz="'.$quiz_id.'" data-user="'.$user_id.'">'.__('Mark Quiz as Checked','vibe').'</a>';
      endif;
          ...
          foreach($fields as $k=>$field){
            switch($field){
              ...
              case 'stats_student_certificate_code':
                $title=__('CERTIFICATE CODE','vibe');
                if(!in_array($title,$csv_title))
                $csv_title[$i]=$title;
          ...
          echo '</div>';
                  ...
                  if($prev >=0){
                    ...
                    if(bp_course_get_post_type($units[$prev]) == 'quiz'){
                      echo '<a href="#" id="prev_quiz" data-unit="'.$units[$prev].'" class="unit '.$unit_class.'"><span>'.__('Previous Quiz','vibe').'</span></a>';
                    }else{
                      ...
                      if(get_post_type($units[$prev]) == 'wplms-assignment'){
                        echo '<a href="#" id="prev_unit" data-unit="'.$units[$prev].'" class="unit unit_button"><span>'.__('Previous Assignment','vibe').'</span></a>';
                      }
                    }
                    ...
                    if(bp_course_get_post_type($units[$next]) == 'quiz'){
                        ...
                        if($quiz_passing_flag){
                            ...
                            if(get_post_type($units[$next]) == 'wplms-assignment'){
                              echo '<a href="#" id="next_unit" '.(($hide_unit)?'':'data-unit="'.$units[$next].'"').' class="unit '.$unit_class.'"><span>'.__('Next Assignment','vibe').'</span></a>';
                            }
                        }
                    }
