...
function bp_course_description(){
	...
	function bp_course_get_avatar( $args = array() ) {
		...
		$r = wp_parse_args( $args, $defaults );
		$r = apply_filters('bp_course_get_avatar_args',$r);
		...
		return apply_filters('bp_course_get_avatar',$thumb,$args);
	}
...
function bp_course_get_question_details($question_id,$force = null){
	...
	$question['hint'] = apply_filters('the_content',get_post_meta($question_id,'vibe_question_hint',true));
	$question['explanation'] = apply_filters('the_content',get_post_meta($question_id,'vibe_question_explanation',true));
	...
	if(!empty($question['options'])){
		foreach ($question['options'] as $key => $op) {
			$options_with_the_content[]= apply_filters('the_content',$op);
		}
	}
			...
			if(!empty($question['content'])){
				...
				echo '<li>
					...
					if($question['type'] == 'survey'){
						...
						echo '<span> '.__('Total Marks :','vibe').' '.apply_filters('wplms_quiz_results_quesstion_total_marks',$question['max_marks'],$quiz_id,$question,$user_id).'</span>';
						$total_sum += $question['max_marks'];


						if(isset($question['marks']) ){
							if($question['marks'] > 0){
								echo '<span>'.__('MARKS OBTAINED','vibe').' <i class="icon-check"></i> '.apply_filters('wplms_quiz_results_quesstion_user_marks',$question['marks'],$quiz_id,$question,$user_id).'</span>';
							}else{
								echo '<span>'.__('MARKS OBTAINED','vibe').' <i class="icon-x"></i> '.apply_filters('wplms_quiz_results_quesstion_user_marks',$question['marks'],$quiz_id,$question,$user_id).'</span>';
							}
							$sum += intval($question['marks']);
						}else{
			...
			foreach($questions['ques'] as $key=>$question){
				if(isset($question) && is_numeric($question)){
					...
					if($type == 'survey'){
						...
						echo '<span> '.__('Total Marks :','vibe').' '.apply_filters('wplms_quiz_results_quesstion_total_marks',$questions['marks'][$key],$quiz_id,$question,$user_id).'</span>';
						...
						if(isset($marks) && $marks !=''){
							...
							$user_result[$question]['marks']=$marks;
							$sum = apply_filters('vibe_marks_roundoff',($sum+intval($marks)),$sum,$marks);
						}else{
							echo '<span>'.__('Marks Obtained','vibe').' <i class="icon-alarm"></i></span>';
						}
