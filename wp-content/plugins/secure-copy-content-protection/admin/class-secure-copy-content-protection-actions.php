	...
	public function store_data( $data ) {
		...
		if (isset($data["sccp_action"]) && wp_verify_nonce($data["sccp_action"], 'sccp_action')) {
			...
			$options           = array(
				...
				"bc_header_text"        => isset($data['sccp_bc_header_text']) ? stripslashes($data['sccp_bc_header_text']) : __('You cannot copy content of this page', $this->plugin_name)
			...
			if ($blockcont_pass) {
				foreach ( $blockcont_pass as $key => $value ) {
					...
					$sccp_wpdb_id = absint( intval($bc_result->id));
					$bc_result_options = json_decode($bc_result->options, true);
