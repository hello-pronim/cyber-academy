...
function wplms_dashboard_widget_scripts(){
    ...
    if(in_array($bp->current_component,$components)){ 
		...
		if ( is_active_widget( false, false, 'wplms_instructor_stats', true ) || is_active_widget( false, false, 'wplms_instructor_commission_stats', true )
			|| is_active_widget( false, false, 'wplms_student_stats', true ) || is_active_widget( false, false, 'wplms_customize_instructor_activity', true)) {
			...
			wp_enqueue_script( 'wplms-jquery-ui-js',WPLMS_DASHBOARD_URL.'/js/jquery-ui.js',array('jquery'),true);
			wp_enqueue_style( 'wplms-jquery-ui-css',WPLMS_DASHBOARD_URL.'/css/jquery-ui.css',array('jquery'),true);
		}
		$currencies = array();
		...
		$translation_array = array(
			...
			'no_data' => __('No data found','wplms-dashboard'),
