...
add_action( 'widgets_init', 'wplms_customize_instructor_dash_stats_widget' );

function wplms_customize_instructor_dash_stats_widget() {
    register_widget('wplms_customize_instructor_dash_stats');
}

class wplms_customize_instructor_dash_stats extends WP_Widget {

    /** constructor -- name this the same as the class above */
    function __construct() {
      $widget_ops = array( 'classname' => 'wplms_customize_instructor_dash_stats', 'description' => __('Customize Simple stats scores for instructors', 'wplms-dashboard') );
      $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'wplms_customize_instructor_dash_stats' );
      parent::__construct( 'wplms_customize_instructor_dash_stats', __(' DASHBOARD : Instructor Customize Simple Stats', 'wplms-dashboard'), $widget_ops, $control_ops );
    }
    ...
    echo '<div class="'.$width.'">
          ...
          switch($stats){
            case 'woo_commission':
                ...
                if(!$migrated_to_activity){
                    ...
                    if(!empty($currencies_data) && !empty($total_commission_cur) && count($currencies_data) > 1){
                    $multi_html .= '<div id="vibe-tabs-student-activity" class="tabs tabbable">
                    <ul class="nav nav-tabs clearfix">';
                    foreach ($total_commission_cur as $key => $currency) {
                        $multi_html .= '<li id="'.$key.'" class=""><a href="#total-commission-widget-tab-'.$key.'" data-toggle="tab" class="currency_tabs">'.$key.'</a></li>';
                    }
                    $multi_html .= '</ul>
                    </div>
                    ...
                    if(!empty($total_commission_cur) && !empty($currencies_data)){
                    foreach ($total_commission_cur as $key => $total_commission) {
                
                        $multi_html .= '<div id="total-commission-widget-tab-'.$key.'" class="tab-pane" '.((count($currencies_data) < 2 )?'style="display:block"':"").'>';
                        ...
                        $label=__('Total Commission Earned','wplms-dashboard');
                        $multi_html .= '<h3>'.$value.'<span>'.$label.'</span></h3>';
            ...
            case 'badges': 
                ...
                break;
            case 'certificates':
                $type_icon = '<i class="fa fa-star-o" aria-hidden="true"></i>';
                ...
                break;
            case 'students':
                $type_icon = '<i class="fa fa-users" aria-hidden="true"></i>';
        ...
        echo '<div class="dash-stats" style="color: white;">';
        
        if($stats == 'woo_commission')
          echo '<a class="commission_reload"><i class="icon-reload"></i></a>';
        if($stats == 'woo_commission' && $migrated_to_activity){
          echo $multi_html;
        }else{
            echo '<span class="instructor_dash_icons">'.$type_icon.'</span>';
            echo '<p style="cursor: default;">Total '.$label.' <strong>'.$value.'</strong></p>';   
			
			$user = wp_get_current_user();
			
			$direct_url = get_home_url();
			if ($stats == "students") {
				//$direct_url = get_home_url();
				
				$direct_url = home_url().'/instructor-students/';
				
			} else if ($stats == "courses") {
				//$direct_url = get_home_url(null, 'all-courses', null);
				$direct_url = home_url().'/my-account/'.$user->data->user_nicename.'/course/instructor-courses/';
				
			} else if ($stats == "questions") {
				//$direct_url = get_home_url();
				$direct_url = home_url().'/instructor-questions/';
			} else if ($stats == "certificates") {
				//$direct_url = get_home_url();
				$direct_url = home_url().'/instructor-certificate/';
			}
            ...
            echo '<script>
                (function($){
                    $(".'.$stats.'")
						.mouseover(function() {
							$(this).css("box-shadow", "0px 1px 5px 0px rgba(0,0,0,0.75)");
						})
						.mouseout(function() {
							$(this).css("box-shadow", "0px 1px 5px 0px rgba(0,0,0,0.35)");
						});
    ...
    function form($instance) {  
        $defaults = array( 
                        'title'  => __('Instructor Custom Stats','wplms-dashboard'),
                        'stats' => '',
